...
(() => {
  ...
  const Maximizer = (() => {
    ...
    function isPlayerControlElement(el) {
      ...
      // ★ 엄격한 키워드: 반드시 플레이어 컨트롤에만 해당하는 것들
    ...
    function doMaximizeDirect(video) {
      ...
      if (playerWrapper) {
        // ★ 래퍼 모드: 래퍼를 fullscreen, 내부(컨트롤 포함) 보존
        ...
        // ★ 직접 모드: video를 MAX_CLASS로 fixed
        //   조상 체인의 모든 형제를 무조건 숨김 (컨트롤 판별 불필요)
