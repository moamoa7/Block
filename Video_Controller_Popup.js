...
(function() {
    ...
    function updateFeedback(text) {
        const overlay = getFeedbackOverlay();  // 피드백 오버레이를 가져옵니다.
        overlay.textContent = text;  // 텍스트를 업데이트합니다.
        overlay.style.opacity = 1;  // 오버레이의 투명도를 1로 설정하여 보이게 만듭니다.
    }
    ...
    function updateVideoDraggingState(video) {
        ...
        if (isMobile) {
          ...
          if (isFullscreen) {
            ...
            // 전체화면 상태이거나, 해당 영상이 전체화면 요소의 자식일 때만 드래그 가능
