...
// @version 4.09.9_Stripped_TrustedHTML_Fix_DOM_Create_FixSpeed_FixMultiVideoSpeed
...
                // Only change currentVideo if the interacting video is different
                if (currentVideo !== video) {
                    currentVideo = video;
                    setupVideoDragging(currentVideo);

                    // VCP_FIX: Ensure newly selected video defaults to 1.0x if not already set.
                    // We use a flag on the video element to track initialization.
                    if (!video._vcpSpeedInitialized) {
                        fixPlaybackRate(currentVideo, 1.0);
                        video._vcpSpeedInitialized = true;
                    } else {
...
            // Ensure the first video is initialized to 1.0
            fixPlaybackRate(currentVideo, 1.0);
            currentVideo._vcpSpeedInitialized = true; // Mark as initialized

            // Update UI sliders based on the new rate (which is now 1.0)
            updatePopupSliders();

            if (shouldShowPopup && !isInitialPopupBlocked) {
                //showPopupTemporarily(); // Kept commented as per original script logic for blocking on specific sites
            }
        }
    }
...
        // Initial video list update and speed fix
