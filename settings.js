...
// @namespace    https://com/
// @version      88.2
// @description  비디오 필터 로직 개선: 채도(Saturation) 처리 방식을 SVG로 통합하여 안정성 및 Shadow DOM 호환성 확보
// @match        *://*/*
// @run-at       document-end
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    function resetState() {
        ...
        Object.assign(state, {
            ...
            videoFilterSettings: isMobile ? { ...CONFIG.MOBILE_FILTER_SETTINGS } : { ...CONFIG.DESKTOP_FILTER_SETTINGS },
    ...
    class SvgFilterManager {
        ...
        #updateFilterAttribute(filterIdSuffix, elementTag, attributeName, attributeValue, rootNode = document) {
            if (!this.isInitialized()) return;
            const filterId = `${this.#options.settings.SHARPEN_ID}${filterIdSuffix}`;
            const filterElement = rootNode.getElementById(filterId);
            if (filterElement) {
                const target = filterElement.querySelector(elementTag);
                if (target) target.setAttribute(attributeName, attributeValue);
            }
        }
        updateGamma(gammaValue, rootNode = document) {
            const exponent = (1 / gammaValue).toString();
            const filterId = `${this.#options.settings.SHARPEN_ID}_gamma`;
            const filterElement = rootNode.getElementById(filterId);
            if (filterElement) {
                filterElement.querySelectorAll('feFuncR, feFuncG, feFuncB').forEach(func => func.setAttribute('exponent', exponent));
            }
        }
        updateBlur(stdDeviation, rootNode = document) {
            this.#updateFilterAttribute('_soft', 'feGaussianBlur', 'stdDeviation', stdDeviation, rootNode);
        }
        updateLinear(shadows, highlights, rootNode = document) {
            const slope = (1 + highlights / 100).toString();
            const intercept = (shadows / 200).toString();
            const filterId = `${this.#options.settings.SHARPEN_ID}_linear`;
            const filterElement = rootNode.getElementById(filterId);
            if (filterElement) {
                filterElement.querySelectorAll('feFuncR, feFuncG, feFuncB').forEach(func => {
                    func.setAttribute('slope', slope);
                    func.setAttribute('intercept', intercept);
                });
            }
        }
        updateSaturation(saturationValue, rootNode = document) {
            if (!this.isInitialized()) return;
            const filterId = `${this.#options.settings.SHARPEN_ID}_saturate`;
            const filterElement = rootNode.getElementById(filterId);
            if (filterElement) {
                const matrix = filterElement.querySelector('feColorMatrix');
                if (matrix) {
                    matrix.setAttribute('values', (saturationValue / 100).toString());
                }
            }
        }
        #createElements() {
            ...
            const filters = [
                ...
                { id: `${settings.SHARPEN_ID}_saturate`, children: [createSvgElement('feColorMatrix', { type: 'saturate', values: (settings.SATURATION_VALUE / 100).toString() })] }
            ];
            svg.append(...filters.map(f => createSvgElement('filter', { id: f.id }, ...f.children)));
            const style = document.createElement('style'); style.id = styleId;
            style.textContent = `.${className}{filter:url(#${filters[4].id}) url(#${filters[2].id}) url(#${filters[0].id}) url(#${filters[1].id}) url(#${filters[3].id})!important;}.${'vsc-gpu-accelerated'}{transform:translateZ(0);will-change:transform;}`;
    ...
    function applyCurrentVideoFilterSettings() {
        if (!filterManager.isInitialized()) return;
        const { GAMMA_VALUE, BLUR_STD_DEVIATION, SHADOWS_VALUE, HIGHLIGHTS_VALUE, SATURATION_VALUE } = state.videoFilterSettings;

        const allRoots = [document, ...(window._shadowDomList_ || []).map(r => r.deref()).filter(Boolean)];
        allRoots.forEach(root => {
            filterManager.updateGamma(GAMMA_VALUE, root);
            filterManager.updateBlur(BLUR_STD_DEVIATION, root);
            filterManager.updateLinear(SHADOWS_VALUE, HIGHLIGHTS_VALUE, root);
            filterManager.updateSaturation(SATURATION_VALUE, root);
        });
    }
    ...
    function setVideoFilterLevel(level) {
        ...
        const allRoots = [document, ...(window._shadowDomList_ || []).map(r => r.deref()).filter(Boolean)];
        allRoots.forEach(root => filterManager.setSharpenMatrix(newMatrix, root));
    ...
    function setImageFilterLevel(level) {
        ...
        const allRoots = [document, ...(window._shadowDomList_ || []).map(r => r.deref()).filter(Boolean)];
        allRoots.forEach(root => imageFilterManager.setSharpenMatrix(newMatrix, root));
    ...
    const uiManager = (() => {
        const styleRules = [
            ...
            `#vsc-video-controls .vsc-submenu { width: ${isMobile ? '220px' : '280px'}; max-width: 80vw; }`,
    ...
    const speedSlider = (() => {
        ...
        function renderControls() {
            ...
            // --- Video Controls (Re-vamped) ---
            const { group: videoGroup, subMenu: videoSubMenu } = createControlGroup('vsc-video-controls', '✨', '영상 선명도/필터');
            const videoOpts = [{ value: "0", text: "꺼짐" }, ...Array.from({ length: 5 }, (_, i) => ({ value: (i + 1).toString(), text: `${i + 1}단계` }))];
            const videoFilterSelect = createSelectControl('선명도', videoOpts, setVideoFilterLevel, 'videoFilterSelect');

            const gammaSlider = createSliderControl('감마', 'gammaSlider', 0.5, 1.5, 0.01, state.videoFilterSettings.GAMMA_VALUE, '');
            const blurSlider = createSliderControl('블러', 'blurSlider', 0, 2, 0.1, state.videoFilterSettings.BLUR_STD_DEVIATION, '');
            const shadowsSlider = createSliderControl('어둡게', 'shadowsSlider', -50, 50, 1, state.videoFilterSettings.SHADOWS_VALUE, '');
            const highlightsSlider = createSliderControl('밝게', 'highlightsSlider', -50, 50, 1, state.videoFilterSettings.HIGHLIGHTS_VALUE, '');
            const saturationSlider = createSliderControl('채도', 'saturationSlider', 0, 200, 1, state.videoFilterSettings.SATURATION_VALUE, '%');
            const videoResetBtn = createButton('vsc-video-reset', '필터 초기화', '필터 초기화', 'vsc-btn');

            videoSubMenu.append(
                videoFilterSelect,
                gammaSlider.controlDiv,
                blurSlider.controlDiv,
                shadowsSlider.controlDiv,
                highlightsSlider.controlDiv,
                saturationSlider.controlDiv,
                videoResetBtn
            );

            gammaSlider.slider.oninput = () => {
                const val = parseFloat(gammaSlider.slider.value);
                state.videoFilterSettings.GAMMA_VALUE = val;
                gammaSlider.valueSpan.textContent = val.toFixed(2);
                applyCurrentVideoFilterSettings();
            };
            blurSlider.slider.oninput = () => {
                const val = parseFloat(blurSlider.slider.value);
                state.videoFilterSettings.BLUR_STD_DEVIATION = val.toFixed(1);
                blurSlider.valueSpan.textContent = val.toFixed(1);
                applyCurrentVideoFilterSettings();
            };
            shadowsSlider.slider.oninput = () => {
                const val = parseInt(shadowsSlider.slider.value, 10);
                state.videoFilterSettings.SHADOWS_VALUE = val;
            ...
            highlightsSlider.slider.oninput = () => {
                ...
                state.videoFilterSettings.HIGHLIGHTS_VALUE = val;
            ...
            saturationSlider.slider.oninput = () => {
                ...
                state.videoFilterSettings.SATURATION_VALUE = val;
                saturationSlider.valueSpan.textContent = `${val}%`;
                applyCurrentVideoFilterSettings();
            };
            videoResetBtn.onclick = () => {
                const defaults = isMobile ? CONFIG.MOBILE_FILTER_SETTINGS : CONFIG.DESKTOP_FILTER_SETTINGS;
                state.videoFilterSettings = { ...defaults };

                gammaSlider.slider.value = defaults.GAMMA_VALUE;
                gammaSlider.valueSpan.textContent = parseFloat(defaults.GAMMA_VALUE).toFixed(2);

                blurSlider.slider.value = defaults.BLUR_STD_DEVIATION;
                blurSlider.valueSpan.textContent = parseFloat(defaults.BLUR_STD_DEVIATION).toFixed(1);

                shadowsSlider.slider.value = defaults.SHADOWS_VALUE;
                shadowsSlider.valueSpan.textContent = defaults.SHADOWS_VALUE;

                highlightsSlider.slider.value = defaults.HIGHLIGHTS_VALUE;
                highlightsSlider.valueSpan.textContent = defaults.HIGHLIGHTS_VALUE;

                saturationSlider.slider.value = defaults.SATURATION_VALUE;
                saturationSlider.valueSpan.textContent = `${defaults.SATURATION_VALUE}%`;
    ...
    function updateVideoFilterState(video) { if (!filterManager.isInitialized()) return; video.classList.toggle('vsc-video-filter-active', video.dataset.isVisible !== 'false' && (state.currentVideoFilterLevel > 0 || state.videoFilterSettings.SATURATION_VALUE !== 100)); }
        ...
        if (root instanceof ShadowRoot && !root.host.hasAttribute(attr)) {
            ...
            if (svgNode) {
                ...
                if (element.tagName === 'VIDEO') {
                    applyCurrentVideoFilterSettings();
                    manager.setSharpenMatrix(calculateSharpenMatrix(state.currentVideoFilterLevel), root);
                } else {
                    manager.setSharpenMatrix(calculateSharpenMatrix(state.currentImageFilterLevel), root);
                }
            }
        }
    }
