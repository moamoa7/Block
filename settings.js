...
// @namespace    https://com/
// @version      39.5
// @description  일부 라이브 방송 딜레이 조절 기능 추가 / UI/UX 개선 / 전체 코드 안정화
// @match        *://*/*
// @grant        none
...
(function () {
    ...
    const CONFIG = {
        ...
        DELAY_ADJUSTER: {
            ...
            TARGET_DELAY: 1000,
            SPEED_LEVELS: [
                { minDelay: 5000, playbackRate: 1.3 }, { minDelay: 3000, playbackRate: 1.25 },
                { minDelay: 2500, playbackRate: 1.2 }, { minDelay: 2000, playbackRate: 1.15 },
                { minDelay: 1500, playbackRate: 1.1 }, { minDelay: 0, playbackRate: 1.05 }
            ],
            NORMAL_RATE: 1.0
        }
    };
    ...
    const autoDelayManager = (() => {
        ...
        function calculateDelay(videoElement) { if (!videoElement || !videoElement.buffered || videoElement.buffered.length === 0) return null; try { const bufferedEnd = videoElement.buffered.end(videoElement.buffered.length - 1); const delay = bufferedEnd - videoElement.currentTime; return delay >= 0 ? delay * 1000 : null; } catch (e) { return null; } }
        function getPlaybackRate(averageDelay) { for (const config of D_CONFIG.SPEED_LEVELS) { if (averageDelay >= config.minDelay) return config.playbackRate; } return D_CONFIG.SPEED_LEVELS[D_CONFIG.SPEED_LEVELS.length - 1].playbackRate; }
        function adjustPlaybackRate(rate) { if (!video || Math.abs(video.playbackRate - rate) < 0.01) return; safeExec(() => { video.playbackRate = rate; state.currentPlaybackRate = rate; }); }
        function displayDelayInfo(averageDelay) { if (!state.ui.shadowRoot) return; let infoEl = state.ui.shadowRoot.getElementById('vsc-delay-info'); if (!infoEl) { infoEl = document.createElement('div'); infoEl.id = 'vsc-delay-info'; state.ui.shadowRoot.appendChild(infoEl); } const status = state.isDelayAdjusting ? `${state.currentPlaybackRate.toFixed(2)}x` : '1.00x'; infoEl.textContent = `딜레이: ${averageDelay.toFixed(0)}ms (${status})`; }
        ...
        function start() { if (state.delayCheckInterval) return; video = null; state.delayCheckInterval = setInterval(checkAndAdjust, D_CONFIG.CHECK_INTERVAL); }
        function stop() { if (state.delayCheckInterval) { clearInterval(state.delayCheckInterval); state.delayCheckInterval = null; } const infoEl = state.ui.shadowRoot?.getElementById('vsc-delay-info'); if(infoEl) infoEl.remove(); if(video) { safeExec(() => { if(video.playbackRate !== 1.0) video.playbackRate = 1.0; }); video = null;} }
        return { start, stop };
    })();
    ...
    const scanAndApply = () => {
        ...
        if (root) {
            ...
            const speedControls = root.querySelector('.vsc-collapsible');
            if (speedControls) speedControls.style.display = shouldShowSpeedControls ? 'flex' : 'none';
            const toggleButton = root.querySelector('.vsc-btn.toggle');
            ...
            if (state.isUiVisible !== anyMedia) {
                ...
                if (state.isUiVisible) speedSlider.renderControls();
    ...
    // =================================================================================
    // 7. 스크립트 초기화 및 SPA 처리 (Initialization & SPA Handling)
    // =================================================================================
    ...
    function cleanup() {
        safeExec(() => {
            ...
            if (spaNavigationHandler) spaNavigationHandler.cleanup();
    ...
    function start() {
        resetState();
        console.log('🎉 Video_Image_Control (v39.4) Initialized.');
        ...
        // 미디어 감지 전까지 UI를 숨기지 않고 로딩 상태로 보여줌
        speedSlider.show(true);

        const isLive = isLiveStreamPage();
        // setMode는 컨트롤이 렌더링 된 후에 호출되어야 함
        if (isLive) {
        ...
        visibilityChangeListener = () => {
            if (document.hidden) {
                ...
                for (const media of state.activeMedia) { audioManager.suspendContext(); }
            } else {
                scheduleIdleTask(scanAndApply);
                for (const media of state.activeMedia) { audioManager.resumeContext(); }
            }
        };
        ...
        fullscreenChangeListener = () => uiManager.moveUiTo(document.fullscreenElement || document.body);
