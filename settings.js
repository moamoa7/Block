...
// @namespace    https://com/
// @version      69.7 (Mobile Optimization: Apply FX to single visible video)
// @description  On mobile, audio effects are now applied only to the most visible video in the viewport to optimize performance. Includes previous TypeError fix.
// @match        *://*/*
// @run-at       document-end
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    function resetState() {
        ...
        Object.assign(state, {
            ...
            currentlyVisibleMedia: null, // NEW: Track the single visible media on mobile
    ...
    const stereoWideningManager = (() => {
        ...
        return {
            ...
            setWideningFactor: (m, v) => { const n = getOrCreateNodes(m); if(n) setParamWithFade(n.ms_side_gain.gain, v); },
            ...
            setLfoRate: (m, rate) => { const n = getOrCreateNodes(m); if(n) n.lfo.frequency.linearRampToValueAtTime(rate, n.context.currentTime + 0.05); },
            setVolumeFollower,
            setDynamicDepth,
            cleanupForMedia,
            disconnectGraph,
    ...
    // UPDATED: Apply effects to the relevant media (all on desktop, one on mobile)
    function applyAudioEffectsToMedia(mediaSet) {
        mediaSet.forEach(media => {
            ...
            stereoWideningManager.setPan(media, state.currentStereoPan);
            stereoWideningManager.setReverb(media, state.currentReverbMix);
        });
    }

    // UPDATED: Disconnect effects from media
    ...
    const speedSlider = (() => {
        ...
        function renderControls() {
            ...
            wideningSlider.slider.oninput = () => {
                ...
                const mediaToAffect = isMobile && state.currentlyVisibleMedia ? [state.currentlyVisibleMedia] : state.activeMedia;
                mediaToAffect.forEach(m => stereoWideningManager.setWideningFactor(m, val));
            };
            ...
            hpfSlider.slider.oninput = () => {
                ...
                mediaToAffect.forEach(media => {
                    const nodes = state.audioContextMap.get(media);
                    if (nodes) {
                        if (nodes.hpfWiden) nodes.hpfWiden.frequency.linearRampToValueAtTime(val, nodes.context.currentTime + 0.05);
                        if (nodes.hpfSpatial) nodes.hpfSpatial.frequency.linearRampToValueAtTime(val, nodes.context.currentTime + 0.05);
                    }
                });
            };
                ...
                if (!state.isVolumeFollowerEnabled) {
                    ...
                    mediaToAffect.forEach(media => {
                        ...
                        if (nodes && nodes.lfoDepth) {
                            nodes.lfoDepth.gain.linearRampToValueAtTime(val, nodes.context.currentTime + 0.05);
                        }
                    });
                }
            };
            ...
            panSlider.slider.oninput = () => {
                ...
                const mediaToAffect = isMobile && state.currentlyVisibleMedia ? [state.currentlyVisibleMedia] : state.activeMedia;
                mediaToAffect.forEach(m => stereoWideningManager.setPan(m, val));
            };
            ...
            reverbSlider.slider.oninput = () => {
                ...
                const mediaToAffect = isMobile && state.currentlyVisibleMedia ? [state.currentlyVisibleMedia] : state.activeMedia;
                mediaToAffect.forEach(m => stereoWideningManager.setReverb(m, val));
            };
            ...
            lfoRateSlider.slider.oninput = () => {
                ...
                const mediaToAffect = isMobile && state.currentlyVisibleMedia ? [state.currentlyVisibleMedia] : state.activeMedia;
                mediaToAffect.forEach(m => stereoWideningManager.setLfoRate(m, val));
            };
            ...
            resetBtn.onclick = () => {
                ...
                state.currentWideningFactor = defaults.widening; state.currentHpfHz = defaults.hpf;
                state.currentSpatialDepth = defaults.depth; state.currentStereoPan = defaults.pan;
                state.currentReverbMix = defaults.reverb; state.currentReverbLength = defaults.reverbLen;
                state.currentLfoRate = defaults.lfoRate;
                ...
                mediaToAffect.forEach(m => {
                    ...
                    if (!nodes) return;
                    if (nodes.hpfWiden) nodes.hpfWiden.frequency.linearRampToValueAtTime(defaults.hpf, nodes.context.currentTime + 0.05);
                    if (nodes.hpfSpatial) nodes.hpfSpatial.frequency.linearRampToValueAtTime(defaults.hpf, nodes.context.currentTime + 0.05);
                    if(nodes.stereoPanner) nodes.stereoPanner.pan.linearRampToValueAtTime(defaults.pan, nodes.context.currentTime + 0.05);
                    if(nodes.wetGainReverb) nodes.wetGainReverb.gain.linearRampToValueAtTime(defaults.reverb, nodes.context.currentTime + 0.05);
                    if(nodes.lfo) nodes.lfo.frequency.linearRampToValueAtTime(defaults.lfoRate, nodes.context.currentTime + 0.05);
                    if (!state.isVolumeFollowerEnabled && !state.isDynamicDepthEnabled && nodes.lfoDepth) {
                        nodes.lfoDepth.gain.linearRampToValueAtTime(defaults.depth, nodes.context.currentTime + 0.05);
                    }
                });
            };
    ...
    const scanAndApply = () => {
        ...
        // On desktop, apply to all. On mobile, this will be handled by the IntersectionObserver.
        if (!isMobile) {
            allMedia.forEach(m => {
    ...
    function ensureObservers() {
        ...
        if (!intersectionObserver) {
            // This observer handles both visibility for filters and, on mobile, which media gets audio FX.
            intersectionObserver = new IntersectionObserver(entries => {
                ...
                root: null,
                rootMargin: '0px',
                threshold: Array.from({ length: 101 }, (_, i) => i / 100) // For accurate visibility tracking
            });
        }
    }
