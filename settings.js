...
// @namespace   https.com/
// @version     17.1-beta-optimized-fixed (오류 수정 - 미리보기 검은화면 수정/iframe 영상플레이어 차단 수정)
...
            if (MediaStateManager.hasIframe(iframe)) return;
            ...
            // 제안된 변경사항: iframe src가 직접적인 비디오 URL인지 먼저 확인합니다.
...
              logManager.logIframeContext(iframe, '접근 불가 (보호 정책)');
              replaceBlockedIframeUI(iframe);
              return;
            }
...
                if (doc) { initializeAll(doc); logManager.logIframeContext(iframe, '비동기 초기화 성공'); }
                else {
                    logManager.logIframeContext(iframe, '비동기 초기화 실패(타임아웃)');
                    setTimeout(() => initIframe(iframe), 3000);
                }
