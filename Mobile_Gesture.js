...
(function() {
    ...
    const isFullscreenActive = (root) => {
        return !!getFS() || (root && root.classList && root.classList.contains('gt-fullscreen-active'));
    };
    ...
    const updateWrapperOverflow = (video) => {
        ...
        if (isFS || isZoomed) wrapper.style.overflow = 'hidden';
        else wrapper.style.overflow = wrapper.dataset.gtOverflow || 'visible';
    };
    ...
    const getDeviceScale = (containerH, isPreview) => {
        ...
        if ('ontouchstart' in window || navigator.maxTouchPoints > 0) {
            if (logicalW === 0 || Math.abs(logicalW - screenW) > 50) {
                if (screenW < 600) { logicalW = screenW; GM_setValue('gt_logical_w', logicalW); }
                else if (logicalW === 0) { logicalW = 400; }
            }
        }
        let scale = 1.0;
    ...
    const lockOrientation = (dir) => { if (screen.orientation?.lock) screen.orientation.lock(dir).catch(() => {}); };
    ...
    const isExcludedZone = (e) => {
        ...
        if (path.length > 0) {
            for (let el of path) {
                if (el.nodeType === 1) {
                    if (el.hasAttribute && (el.hasAttribute('data-vsc-ui') || el.hasAttribute('data-gt-ui'))) return true;
    ...
    const isNakedForumVideo = (video) => {
        ...
        const pRect = p.getBoundingClientRect(), vRect = video.getBoundingClientRect();
    ...
    const restoreVideoStyle = (video) => {
        ...
        if (video.dataset.gtOrigObjectFit !== undefined) {
            if (video.dataset.gtOrigObjectFit === '') video.style.removeProperty('object-fit');
            else video.style.objectFit = video.dataset.gtOrigObjectFit;
        }
        if (video.gtState) { video.gtState.scale = 1.0; video.gtState.panX = 0; video.gtState.panY = 0; video.style.transform = ''; }
        ...
        if (wrapper && wrapper.classList.contains('gt-video-wrapper')) {
            wrapper.style.height = 'auto'; wrapper.style.maxWidth = '100%';
            wrapper.style.removeProperty('touch-action'); wrapper.style.removeProperty('overscroll-behavior');
            ...
            video.style.width = '100%'; video.style.height = 'auto'; video.style.maxWidth = '100%';
        }
    };
    ...
    const clearAllBlockingStyles = (video) => {
        ...
        if (root && root !== document.body) { root.style.removeProperty('touch-action'); root.style.removeProperty('overscroll-behavior'); }
        const wrapper = video.parentNode;
        if (wrapper && wrapper.classList.contains('gt-video-wrapper')) {
            wrapper.style.removeProperty('touch-action'); wrapper.style.removeProperty('overscroll-behavior');
            wrapper.style.overflow = wrapper.dataset.gtOverflow || 'visible';
    ...
    const getUICss = (S, uid) => {
        ...
        return `
        #${uid} { position: fixed !important; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none !important; z-index: 2147483646 !important; }
        ...
        #${uid}.gt-fs-on .gt-mini-progress { display: block !important; opacity: 0.9 !important; height: 3px !important; }
        ...
        #${uid} .gt-btn-base svg { width: ${22 * FS}px; height: ${22 * FS}px; transition: all 0.2s ease; fill: none !important; stroke: currentColor !important; stroke-width: 2 !important; stroke-linecap: round !important; stroke-linejoin: round !important; }
        ...
        #${uid}.gt-fs-on .gt-btn-base { display: flex !important; }
        #${uid}.gt-fs-on.gt-ui-visible .gt-btn-base { opacity: 0.6 !important; pointer-events: auto !important; }
        #${uid}.gt-fs-on .gt-btn-base.hidden-by-state { display: none !important; pointer-events: none !important; }
        #${uid}.gt-fs-on.gt-ui-visible .gt-btn-base:active { opacity: 1 !important; color: #fff; background: rgba(0,0,0,0.55); }

        #${uid} .gt-pip-btn { top: 30%; left: 10px; }
        #${uid} .gt-rotate-btn { bottom: 30%; left: 10px; }
        #${uid} .gt-fit-btn { top: 30%; right: 10px; }
        #${uid} .gt-reset-zoom-btn { bottom: 30%; right: 10px; }

        #${uid} .gt-seek-msg { position: absolute !important; top: 45% !important; color: rgba(255, 255, 255, 0.95) !important; z-index: 2147483647 !important; pointer-events: none !important; opacity: 0; transition: opacity 0.15s ease-out; display: none !important; flex-direction: row !important; align-items: center !important; justify-content: center !important; gap: ${6 * FS}px !important; font-family: system-ui, sans-serif !important; white-space: nowrap !important; text-shadow: 0 0 ${10 * FS}px rgba(0,0,0,0.8), 0 ${2 * FS}px ${4 * FS}px rgba(0,0,0,0.5) !important; }
        #${uid}.gt-fs-on .gt-seek-msg { display: flex !important; }
        ...
        #${uid} .gt-seek-text { display: block !important; font-size: ${15 * FS}px !important; font-weight: 500 !important; line-height: 1 !important; white-space: nowrap !important; }
        #${uid} .gt-arrows { display: flex !important; flex-direction: row !important; align-items: center !important; font-size: ${22 * FS}px !important; line-height: 1 !important; }
        #${uid} .gt-arrows span { display: block !important; line-height: 1 !important; }

        #${uid} .gt-toast { position: absolute; top: 10%; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.5); color: #fff; padding: ${5 * FS}px ${12 * FS}px; border-radius: ${4 * FS}px; font: 700 ${14 * FS}px system-ui; z-index: 2147483647; pointer-events: none; opacity: 0; transition: opacity 0.2s; text-shadow: 0 0 ${2 * FS}px #000; display: none; }
        #${uid}.gt-fs-on .gt-toast { display: block !important; }
    ...
    const getMountTarget = () => {
        const fs = getFS();
        if (fs) {
            if (fs.tagName === 'VIDEO') return fs.parentElement || document.documentElement;
            return fs;
        }
    ...
    const syncFsClass = (video) => {
        ...
        const isFS = isFullscreenActive(video.gtRoot);
        video.gtUI.classList.toggle('gt-fs-on', isFS);
    };

    const mountHost = (video) => {
        if (!video || !video.gtHost) return;
        const target = getMountTarget();
        if (video.gtHost.parentNode !== target) {
            try { target.appendChild(video.gtHost); } catch (e) {}
        }
        syncFsClass(video);
    };
    ...
    const showMsg = (txt, video = null) => {
        ...
        if (!uiLayer) return;
        let t = uiLayer.querySelector('.gt-toast');
        if (!t) {
            if (!txt) return;
            t = document.createElement('div'); t.className = 'gt-toast'; uiLayer.appendChild(t);
        }
        if (!txt) {
    ...
    const identify = (e) => {
        let targetVideo = null, rootContainer = null;
        let cx = e.clientX, cy = e.clientY;
        if (e.touches && e.touches.length > 0) { cx = e.touches[0].clientX; cy = e.touches[0].clientY; }
        else if (e.changedTouches && e.changedTouches.length > 0) { cx = e.changedTouches[0].clientX; cy = e.changedTouches[0].clientY; }

        const path = (e.composedPath && typeof e.composedPath === 'function') ? e.composedPath() : [];
        for (let el of path) { if (el.tagName === 'VIDEO') { targetVideo = el; break; } }

        if (!targetVideo) {
            const fsEl = getFS();
            if (fsEl) { if (fsEl.tagName === 'VIDEO') targetVideo = fsEl; else targetVideo = findDeepVid(fsEl); }
        }
        ...
        if (!targetVideo && cx !== undefined && cy !== undefined) {
            const scan = (root) => {
                ...
                let els = root.querySelectorAll ? root.querySelectorAll('*') : [];
                for (let el of els) { if (el.shadowRoot) { let res = scan(el.shadowRoot); if (res) return res; } }
                return null;
            };
            targetVideo = scan(document);
        }
        ...
        if (targetVideo.gtRoot && document.contains(targetVideo.gtRoot)) {
            ...
            if (rootContainer && rootContainer !== document.body && rootContainer !== document.documentElement) targetVideo.gtRoot = rootContainer;
        }
        if (!rootContainer || rootContainer === document.body || rootContainer === document.documentElement) rootContainer = targetVideo.parentNode || targetVideo;
        if (rootContainer && rootContainer.tagName === 'VIDEO') rootContainer = rootContainer.parentNode;

        if (e.touches && e.touches.length > 0) {
            const isFS = isFullscreenActive(rootContainer);
            if (!isFS) {
    ...
    const buildUI = (root, video) => {
        ...
        host.id = 'gt-host-' + uid;
        host.setAttribute('data-gt-ui', '1');
        host.style.cssText = 'all:initial!important;position:fixed!important;top:0!important;left:0!important;width:0!important;height:0!important;z-index:2147483646!important;pointer-events:none!important;';
    ...
    const checkAndBuildUI = (root, video) => {
        ...
        if (video.gtUI && video.gtHost && document.contains(video.gtHost)) {
            applyFixedScale(root, video, video.gtUI);
            mountHost(video);
            return video.gtUI;
        }
        if (video.gtHost && !document.contains(video.gtHost)) {
            try { video.gtHost.remove(); } catch (e) {}
            video.gtHost = null; video.gtUI = null;
        }
        return buildUI(root, video);
    };
    ...
    const setupPlayer = (video) => {
        ...
        if (isNaked && video.parentNode && !video.parentNode.classList.contains('gt-video-wrapper')) {
            ...
            video.style.margin = '0'; video.style.width = '100%'; video.style.height = 'auto'; video.style.maxWidth = '100%';
    ...
    const onStart = (e) => {
        ...
        if (curIsFS && !targetP.classList.contains('gt-fullscreen-active')) targetP.classList.add('gt-fullscreen-active');
        syncFsClass(targetV);

        clearTimeout(lpTimer); const now = Date.now();
        const isRapid = (now - lastTapTime < 350);
        if (!isRapid) { tapCount = 1; if (curIsFS) wakeUpUI(targetP, targetV); }
        else tapCount++;
        lastTapTime = now;
    ...
    const onMove = (e) => {
        ...
        if (!action) {
            if (Math.abs(dx) > CFG.minDist || Math.abs(dy) > CFG.minDist) {
                ...
                if (!isFS) { action = 'scroll_pass'; isTouch = false; return; }
                if (Math.abs(dx) > Math.abs(dy)) { action = 'seek'; if (e.cancelable) e.preventDefault(); hideUI(targetV); }
                else action = 'ignore';
            } else return;
        }
        ...
        document.addEventListener(evt, () => {
            ...
            if (!fsEl) {
                ...
                document.querySelectorAll('video').forEach(v => {
                    if (v.parentNode && v.parentNode.classList.contains('gt-video-wrapper')) v.gtRoot = v.parentNode;
                    restoreVideoStyle(v);
                    clearAllBlockingStyles(v);
                    if (v.gtHost) { try { document.documentElement.appendChild(v.gtHost); } catch (e) {} }
                    syncFsClass(v);
                });
                setTimeout(() => {
                    document.querySelectorAll('video').forEach(v => {
                        clearAllBlockingStyles(v);
                        if (v.gtUI && v.gtRoot) applyFixedScale(v.gtRoot, v, v.gtUI);
                        if (v.gtHost && v.gtHost.parentNode !== document.documentElement) { try { document.documentElement.appendChild(v.gtHost); } catch (e) {} }
                        syncFsClass(v);
                    });
                }, 200);
            } else {
                setTimeout(() => {
                    let v = targetV;
                    if (!v || !v.isConnected) v = fsEl.tagName === 'VIDEO' ? fsEl : findDeepVid(fsEl);
                    ...
                    checkAndBuildUI(v.gtRoot || newRoot, v);
                    mountHost(v);
