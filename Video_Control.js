...
(function () {
    ...
    const TUNING = {
        ...
        BOOST_RATE: 1.03,
        TRIGGER_MARGIN_MS: 1500,      // 1000 → 1500 (target+1.5초 넘어야 boost)
        SETTLE_MARGIN_MS: 300,        // 200 → 300
        ...
        // ── 새로 추가: 최소 버퍼 보호 ──
        MIN_BUFFER_SEC: 2.0,          // 이 이하로 버퍼가 떨어지면 boost 즉시 중단
    };

    const IS_CHZZK = location.hostname.includes('chzzk.naver.com');
    const PLATFORM_OFFSET = IS_CHZZK ? 500 : 0;
    const RECOMMENDED_PRESETS = IS_CHZZK ? [2, 3] : [3, 4];  // 추천 프리셋 상향

    // 기본 목표값 상향: CHZZK 3초, SOOP 4초
    const DEFAULT_TARGET_MS = IS_CHZZK ? 3000 : 4000;
    ...
    const computeColor = (() => {
        ...
        return (diff) => {
            ...
            lastInput = rounded;
            const ratio = clamp(diff / 3000, 0, 1);   // 2000 → 3000 (색상 범위 넓힘)
    ...
    let targetDelayMs = loadConfig().targetDelayMs ?? DEFAULT_TARGET_MS;
    ...
    function drawGraph() {
        ...
        // 최소 버퍼 라인
        const minBufY = h - (TUNING.MIN_BUFFER_SEC * 1000 / 8000) * h;
        graphCtx.strokeStyle = 'rgba(231,76,60,.2)';
        graphCtx.beginPath();
        graphCtx.moveTo(0, minBufY);
        graphCtx.lineTo(w, minBufY);
        graphCtx.stroke();
    ...
    function buildDebugString(bufEnd) {
        ...
        const now = performance.now();
        const bufAhead = bufEnd >= 0 ? (bufEnd - video.currentTime).toFixed(2) : '-';
        let s = `${video.paused ? '⏸' : '▶'} ct:${video.currentTime.toFixed(2)} buf:${bufAhead}s rate:${effectiveRate.toFixed(3)}`;
    ...
    function processLiveVideo() {
        ...
        /* ── 버퍼 부족 감지: boost 보호 ── */
        const bufferAheadSec = bufEnd - video.currentTime;
        if (bufferAheadSec < TUNING.MIN_BUFFER_SEC) {
            lowBufferCount++;
            // 버퍼 부족 시 boost 즉시 중단
            if (boostActive) {
                stopBoost(now);
                boostEndTime = now + 8000; // 8초 추가 쿨다운
                if (debugVisible) dmLog(`버퍼 부족(${bufferAheadSec.toFixed(1)}s) BOOST 중단`);
            }
        } else {
            lowBufferCount = 0;
        }
        ...
        if (boostActive) {
            ...
            // 버퍼가 최소치 이하면 boost 진입 금지
            if (avg > triggerMs && cooldownOk && bufferAheadSec >= TUNING.MIN_BUFFER_SEC + 1.0) {
                triggerConfirmCount++;
            } else {
                triggerConfirmCount = 0;
            }
    ...
    function createPanel() {
        ...
        GM_addStyle(`
            @layer dm {
                ...
                .dm-hint{font-size:9px;color:#888;margin-top:4px;line-height:1.3}
                @keyframes dm-pulse{
        ...
        panel.innerHTML = `
            ...
            <div class="dm-hint" data-dm="hint">끊김이 있으면 목표를 높여보세요 (추천: 3~4초)</div>
        ...
        els = {
            ...
            closeBtn: map.close, hint: map.hint,
        };
    ...
    const SHORTCUTS = new Map([
        ...
        ['KeyI',      () => {
            ...
            console.table({
                ...
                seekCount, lastSeekReason, stallCount, lowBufferCount,
                warmup: warmupDone,
                fps: estimatedFps,
                bufAhead: edge ? (edge.end - video.currentTime).toFixed(2) + 's' : '-',
                history: hist.join(','),
