// ==UserScript==
// @name         Video_Control (v170.9.1 - Zero-Idle WebGL & Adaptive SVG)
...
(function () {
  ...
  function VSC_MAIN() {
    ...
    const SHADOW_LRU_MAX = 12; const SPA_RESCAN_DEBOUNCE_MS = 220;
    const GUARD = Object.freeze({ WEBGL_FAIL_COOLDOWN: 5000, WEBGL_FAIL_THRESHOLD: 3, AUDIO_SRC_COOLDOWN: 5000, TARGET_HYSTERESIS_MS: 650, TARGET_HYSTERESIS_MARGIN: 0.8 });
    ...
    function findWebkitPiPVideo() { if (!('webkitPresentationMode' in HTMLVideoElement.prototype)) return null; for (const root of walkRoots(document.body || document.documentElement)) { const vids = root.querySelectorAll?.('video'); if (!vids) continue; for (const v of vids) { try { if (v.webkitPresentationMode === 'picture-in-picture') return v; } catch (_) {} } } return null; }
...
function createTargeting() {
    ...
    function composeVideoParamsInto(out, vUser, isHdr) { composeBaseVideoParams(out, vUser); applyShadowBandStack(out, vUser.shadowBandMask); applyBrightStepStack(out, vUser.brightStepLevel); const autoMods = window.__VSC_INTERNAL__?.AutoScene?.getMods?.() || { br: 1.0, ct: 1.0, sat: 1.0 }; out.gain = (out.gain || 1.0) * autoMods.br; out.contrast = (out.contrast || 1.0) * autoMods.ct; out.satF = (out.satF || 1.0) * autoMods.sat; if (isHdr) { out.bright = (out.bright || 0) * 0.65; out.contrast = 1.0 + ((out.contrast || 1.0) - 1.0) * 0.75; out.satF = 1.0 + ((out.satF || 1.0) - 1.0) * 0.85; } return out; }
    ...
    function createRegistry(scheduler) {
      ...
      function pruneBatch(set, visibleSet, dirtySet, unobserveFn, limit = 200) { let removed = 0, scanned = 0; for (const el of set) { if (++scanned > limit) break; if (!el || !el.isConnected) { set.delete(el); visibleSet.delete(el); dirtySet.delete(el); try { unobserveFn(el); } catch (_) {} if (ro) try { ro.unobserve(el); } catch (_) {} removed++; } } return removed; }
      return { videos, visible, rev: () => rev, refreshObservers, prune: () => { const removed = pruneBatch(videos, visible.videos, dirty.videos, (el) => { if (io) io.unobserve(el); }, 220); if(removed) rev++; }, consumeDirty: () => { const out = dirty; dirty = (dirty === dirtyA) ? dirtyB : dirtyA; dirty.videos.clear(); return out; }, rescanAll: () => { for (const r of walkRoots(document.body || document.documentElement)) { WorkQ.enqueue(r); } } };
    }
function createAudio(sm) {
    ...
    function createFiltersVideoOnly(Utils, config) {
      ...
      function prepare(video, s) {
        ...
        let filterGrade;
        if (!detailOn) filterGrade = 'L';
        else if (isUltraRes) filterGrade = 'M';
        else if (isHighRes) filterGrade = 'M';
        else filterGrade = 'F';
  ...
  outColor = vec4(clamp(color, 0.0, 1.0), 1.0);
      ...
      class WebGLPipeline {
        ...
        syncCanvasPresentationFromVideo(video, now) {
          if (!this._styleDirty && (now - this._lastStyleSyncT) < 500) return;
    ...
    function showOSD(text, durationMs = 1200) {
      ...
      let el = root.querySelector ? root.querySelector('#vsc-osd') : document.getElementById('vsc-osd');
      if (!el) {
    ...
    let __lastSigTarget = null, __lastSigRMode = '', __lastSigPb = false, __lastSigRate = 1, __lastSigFx = false, __lastSigSRev = -1;
    function lightSigChanged(activeTarget, rMode, pbActive, desiredRate, videoFxOn, storeRev) {
      if (storeRev !== __lastSigSRev || activeTarget !== __lastSigTarget || rMode !== __lastSigRMode || pbActive !== __lastSigPb || desiredRate !== __lastSigRate || videoFxOn !== __lastSigFx) {
        __lastSigTarget = activeTarget; __lastSigRMode = rMode; __lastSigPb = pbActive; __lastSigRate = desiredRate; __lastSigFx = videoFxOn; __lastSigSRev = storeRev; return true;
      }
      return false;
    }
    ...
    function createAppController({ Store, Registry, Scheduler, ApplyReq, Adapter, Audio, UI, Utils, P, Targeting }) {
      ...
      Scheduler.registerApply((force) => {
        try {
          ...
          if (!force && vidsDirty.size === 0 && !lightSigChanged(__activeTarget, rMode, pbActive, desiredRate, videoFxOn, sRev)) return;
    ...
    function saveSettings(Store, P) { try { const data = { presetS: Store.get(P.V_PRE_S), presetB: Store.get(P.V_PRE_B), presetMix: Store.get(P.V_PRE_MIX), shadowBandMask: Store.get(P.V_SHADOW_MASK), brightStepLevel: Store.get(P.V_BRIGHT_STEP), renderMode: Store.get(P.APP_RENDER_MODE), audioEnabled: Store.get(P.A_EN), audioBoost: Store.get(P.A_BST), autoScene: Store.get(P.APP_AUTO_SCENE), zoomEn: Store.get(P.APP_ZOOM_EN), playbackRate: Store.get(P.PB_RATE), playbackEnabled: Store.get(P.PB_EN) }; localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); } catch (_) {} }
    function loadSettings(Store, P) { try { const raw = localStorage.getItem(STORAGE_KEY); if (!raw) return; const data = JSON.parse(raw); if (data.presetS) Store.set(P.V_PRE_S, data.presetS); if (data.presetB) Store.set(P.V_PRE_B, data.presetB); if (data.presetMix != null) Store.set(P.V_PRE_MIX, data.presetMix); if (data.shadowBandMask != null) Store.set(P.V_SHADOW_MASK, data.shadowBandMask); if (data.brightStepLevel != null) Store.set(P.V_BRIGHT_STEP, data.brightStepLevel); if (data.renderMode) Store.set(P.APP_RENDER_MODE, data.renderMode); if (data.audioEnabled != null) Store.set(P.A_EN, data.audioEnabled); if (data.audioBoost != null) Store.set(P.A_BST, data.audioBoost); if (data.autoScene != null) Store.set(P.APP_AUTO_SCENE, data.autoScene); if (data.zoomEn != null) Store.set(P.APP_ZOOM_EN, data.zoomEn); if (data.playbackRate != null) Store.set(P.PB_RATE, data.playbackRate); if (data.playbackEnabled != null) Store.set(P.PB_EN, data.playbackEnabled); } catch (_) {} }
    ...
    const saveDebounced = createDebounced(() => saveSettings(Store, P), 500);
    ...
    waitForVisibility().then(() => {
      ...
      onWin('keydown', async (e) => {
        ...
        if (e.altKey && e.shiftKey && e.code === 'Slash') { e.preventDefault(); showOSD('Alt+Shift+V: UI | S: 샤프 | A: 오디오 | R: 렌더 | P: PiP | I: 정보', 3500); return; }
        if (e.altKey && e.shiftKey && e.code === 'KeyI') { e.preventDefault(); const v = __VSC_APP__?.getActiveVideo(); if (!v) { showOSD('활성 비디오 없음'); return; } const w = v.videoWidth, h = v.videoHeight; const fps = v.getVideoPlaybackQuality?.()?.totalVideoFrames ? Math.round(v.getVideoPlaybackQuality().totalVideoFrames / Math.max(0.1, v.currentTime)) : '?'; const dropped = v.getVideoPlaybackQuality?.()?.droppedVideoFrames ?? '?'; const mode = Store.get(P.APP_RENDER_MODE); const st = getVState(v); const backend = st.fxBackend || 'none'; showOSD(`${w}×${h} | ~${fps}fps | drop:${dropped} | ${backend.toUpperCase()}`, 3000); return; }
