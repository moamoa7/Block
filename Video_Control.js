// ==UserScript==
// @name         Video_Control (v170.8.0 - Zero-Idle WebGL & Adaptive SVG)
...
(function () {
  ...
  function VSC_MAIN() {
    ...
    function on(target, type, fn, opts = {}) {
      const merged = { ...opts };
      try { merged.signal = __globalSig; target.addEventListener(type, fn, merged); } catch (_) { try { target.addEventListener(type, fn, opts); } catch (__) {} }
    }
    ...
    function tempToRgbGain(temp) {
      const clamp = VSC_CLAMP; const t = clamp((Number(temp) || 0) / 50, -1, 1);
      let r = 1 + 0.10 * t, b = 1 - 0.10 * t, g = 1 - 0.04 * Math.abs(t);
      const m = Math.max(r, g, b); return { rs: r / m, gs: g / m, bs: b / m };
    }

    const VSC_MEDIA = (() => { let hdr = 0; try { if (window.matchMedia) { const mql = matchMedia('(dynamic-range: high)'); hdr = mql.matches ? 1 : 0; mql.addEventListener('change', (e) => { hdr = e.matches ? 1 : 0; }, { passive: true }); } } catch (_) {} return Object.freeze({ isHdr: () => hdr === 1 }); })();

    const DEFENSE_PRESET = 'safe';
    const VSC_DEFENSE = Object.freeze( DEFENSE_PRESET === 'simple' ? { webglCooldown: false, audioCooldown: false, autoSceneDrmBackoff: false, hideAmbientGlow: false } : { webglCooldown: true, audioCooldown: true, autoSceneDrmBackoff: true, hideAmbientGlow: true } );

    if (VSC_DEFENSE.hideAmbientGlow) { const style = document.createElement('style'); style.textContent = `#cinematics, .ytp-glow-effect, .ytp-glow-canvas-container, [id^="ambient-"] { display: none !important; contain: strict !important; }`; (document.head || document.documentElement).appendChild(style); }
    ...
    function createVideoState() { return { visible: false, rect: null, ir: 0, bound: false, rateState: null, audioFailUntil: 0, applied: false, fxBackend: null, desiredRate: undefined, lastFilterUrl: null, rectT: 0, rectEpoch: -1, fsPatched: false, webglFailCount: 0, webglDisabledUntil: 0, webglTainted: false }; }
    ...
    const DEFAULTS = { video: { presetS: 'off', presetB: 'brOFF', presetMix: 1.0, shadowBandMask: 0, brightStepLevel: 0 }, audio: { enabled: false, boost: 6 }, playback: { rate: 1.0, enabled: false }, app: { active: true, uiVisible: false, applyAll: false, renderMode: 'svg', zoomEn: false, autoScene: false, advanced: false } };
    ...
    if (FEATURE_FLAGS.trackShadowRoots) {
      (function patchAttachShadowOnce() {
        try { const proto = Element.prototype; if (!proto.attachShadow) return; const VSC_PATCH = Symbol.for('vsc.patch.attachShadow'); if (proto[VSC_PATCH]) return; const desc = Object.getOwnPropertyDescriptor(proto, 'attachShadow'), orig = desc && desc.value; if (typeof orig !== 'function') return; try { Object.defineProperty(proto, VSC_PATCH, { value: true }); } catch (_) { proto[VSC_PATCH] = true; } function wrappedAttachShadow(init) { const shadow = orig.call(this, init); try { if (shadow && init && init.mode === 'open') { document.dispatchEvent(new CustomEvent('vsc-shadow-root', { detail: shadow })); } } catch (_) {} return shadow; } try { Object.defineProperty(wrappedAttachShadow, 'toString', { value: Function.prototype.toString.bind(orig), configurable: true }); } catch (_) {} if (desc && desc.configurable === false && desc.writable === false) return; Object.defineProperty(proto, 'attachShadow', { ...desc, value: wrappedAttachShadow }); } catch (e) { log.warn('attachShadow patch failed:', e); }
      })();
    }

    const TOUCHED = { videos: new Set(), rateVideos: new Set() }; function touchedAddLimited(set, el, onEvict) { if (!el) return; if (set.has(el)) { set.delete(el); set.add(el); return; } set.add(el); if (set.size <= CONFIG.TOUCHED_MAX) return; const it = set.values(); const dropN = Math.ceil(CONFIG.TOUCHED_MAX * 0.25); for (let i = 0; i < dropN; i++) { const v = it.next().value; if (v == null) break; set.delete(v); try { onEvict && onEvict(v); } catch (_) {} } }
    ...
    function getRectCached(v, now, maxAgeMs = 420) { const st = getVState(v); const t0 = st.rectT || 0; let r = st.rect; const epoch = st.rectEpoch || 0; if (!r || (now - t0) > maxAgeMs || epoch !== __vscRectEpoch) { r = v.getBoundingClientRect(); st.rect = r; st.rectT = now; st.rectEpoch = __vscRectEpoch; } return r; }
    ...
    function initSpaUrlDetector(onChanged) { if (window.__VSC_SPA_PATCHED__) return; window.__VSC_SPA_PATCHED__ = true; let lastHref = location.href; const emitIfChanged = () => { const next = location.href; if (next === lastHref) return; lastHref = next; onChanged(); }; const wrap = (name) => { const orig = history[name]; if (typeof orig !== 'function') return; history[name] = function (...args) { const ret = Reflect.apply(orig, this, args); queueMicrotask(emitIfChanged); return ret; }; }; wrap('pushState'); wrap('replaceState'); onWin('popstate', emitIfChanged, { passive: true }); }

    const __VSC_INJECT_SOURCE = `;(${VSC_MAIN.toString()})();`;
    function watchIframes() { const inject = (ifr) => { if (!ifr) return; const tryInject = () => { try { const win = ifr.contentWindow; const doc = ifr.contentDocument || win?.document; if (!win || !doc) return; if (win.__VSC_BOOT_LOCK__) return; const host = doc.head || doc.documentElement; if (!host) return; const s = doc.createElement('script'); s.textContent = __VSC_INJECT_SOURCE; host.appendChild(s); s.remove?.(); } catch (_) {} }; tryInject(); if (!ifr.__vscLoadHooked) { ifr.__vscLoadHooked = true; ifr.addEventListener('load', tryInject, { passive: true }); } }; document.querySelectorAll("iframe").forEach(inject); const mo = new MutationObserver((muts) => { for (const m of muts) { if (m.addedNodes) { m.addedNodes.forEach(n => { if (n.tagName === 'IFRAME') inject(n); else if (n.querySelectorAll) n.querySelectorAll('iframe').forEach(inject); }); } } }); mo.observe(document.documentElement, { childList: true, subtree: true }); }
    ...
    function patchFullscreenRequest(video) {
      ...
      const origReq = video.requestFullscreen || video.webkitRequestFullscreen || video.msRequestFullscreen; if (!origReq) return;
      const runWrappedFs = (...args) => { const wrap = ensureFsWrapper(video); const cleanupIfNotFullscreen = () => { const fsEl = document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement; if (!fsEl && fsWraps.has(video)) restoreFromFsWrapper(video); }; if (wrap) { const req = wrap.requestFullscreen || wrap.webkitRequestFullscreen || wrap.msRequestFullscreen; if (typeof req === 'function') { try { const ret = req.apply(wrap, args); if (ret && typeof ret.then === 'function') return ret.catch(err => { cleanupIfNotFullscreen(); throw err; }); return ret; } catch (err) { cleanupIfNotFullscreen(); throw err; } } } try { const ret = origReq.apply(video, args); if (ret && typeof ret.then === 'function') return ret.catch(err => { cleanupIfNotFullscreen(); throw err; }); return ret; } catch (err) { cleanupIfNotFullscreen(); throw err; } };
      if (video.requestFullscreen) patchMethodSafe(video, 'requestFullscreen', function (...args) { return runWrappedFs(...args); }); if (video.webkitRequestFullscreen) patchMethodSafe(video, 'webkitRequestFullscreen', function (...args) { return runWrappedFs(...args); }); if (video.msRequestFullscreen) patchMethodSafe(video, 'msRequestFullscreen', function (...args) { return runWrappedFs(...args); });
    }

    function onFsChange() { const fsEl = document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement; if (!fsEl) { const rootBase = document.documentElement || document.body || document; for (const root of walkRoots(rootBase)) { const vids = root.querySelectorAll?.('video'); if (!vids) continue; vids.forEach(v => { if (fsWraps.has(v)) restoreFromFsWrapper(v); }); } } if (window.__VSC_UI_Ensure) window.__VSC_UI_Ensure(); }
    onDoc('fullscreenchange', onFsChange); onDoc('webkitfullscreenchange', onFsChange);

    const __isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
    function findWebkitPiPVideo() { if (!__isSafari) return null; const rootBase = document.documentElement || document.body || document; for (const root of walkRoots(rootBase)) { const vids = root.querySelectorAll?.('video'); if (!vids) continue; for (const v of vids) { try { if (typeof v.webkitPresentationMode === 'string' && v.webkitPresentationMode === 'picture-in-picture') return v; } catch (_) {} } } return null; }
    let __vscPiPCacheT = 0; let __vscPiPCacheV = null; let __activeDocumentPiPWindow = null, __activeDocumentPiPVideo = null, __pipPlaceholder = null, __pipOrigParent = null, __pipOrigNext = null, __pipOrigCss = '';
    ...
    function getActivePiPVideoSlow() { if (document.pictureInPictureElement instanceof HTMLVideoElement) return document.pictureInPictureElement; if (__activeDocumentPiPWindow && __activeDocumentPiPVideo?.isConnected) return __activeDocumentPiPVideo; try { if (typeof HTMLVideoElement !== 'undefined' && ('webkitPresentationMode' in HTMLVideoElement.prototype)) { const wk = findWebkitPiPVideo(); if (wk) return wk; } } catch (_) {} return null; }
    function getActivePiPVideoCached() { const now = performance.now(); if ((now - __vscPiPCacheT) < 200) return __vscPiPCacheV; __vscPiPCacheT = now; __vscPiPCacheV = getActivePiPVideoSlow(); return __vscPiPCacheV; }
    function getActivePiPVideo() { return getActivePiPVideoCached(); } function isPiPActiveVideo(el) { return !!el && (el === getActivePiPVideoCached()); }

    async function enterPiP(video) {
      if (!video || video.readyState < 2) return false;
      if ('documentPictureInPicture' in window && window.documentPictureInPicture && typeof window.documentPictureInPicture.requestWindow === 'function') { if (__activeDocumentPiPWindow) return true; try { const pipWindow = await window.documentPictureInPicture.requestWindow({ width: Math.max(video.videoWidth / 2, 400), height: Math.max(video.videoHeight / 2, 225) }); __activeDocumentPiPWindow = pipWindow; __activeDocumentPiPVideo = video; __pipOrigParent = video.parentNode; __pipOrigNext = video.nextSibling; __pipOrigCss = video.style.cssText; __pipPlaceholder = document.createElement('div'); __pipPlaceholder.style.width = video.clientWidth + 'px'; __pipPlaceholder.style.height = video.clientHeight + 'px'; __pipPlaceholder.style.background = 'black'; if (__pipOrigParent) __pipOrigParent.insertBefore(__pipPlaceholder, video); pipWindow.document.body.style.margin = '0'; pipWindow.document.body.style.display = 'flex'; pipWindow.document.body.style.justifyContent = 'center'; pipWindow.document.body.style.alignItems = 'center'; pipWindow.document.body.style.background = 'black'; video.style.width = '100%'; video.style.height = '100%'; video.style.objectFit = 'contain'; pipWindow.document.body.append(video); pipWindow.addEventListener('click', () => { if (video.paused) { const p = video.play(); if (p && typeof p.catch === "function") p.catch(() => {}); } else { video.pause(); } }); pipWindow.addEventListener('pagehide', () => { try { video.style.cssText = __pipOrigCss; if (__pipPlaceholder && __pipPlaceholder.parentNode) { __pipPlaceholder.parentNode.insertBefore(video, __pipPlaceholder); __pipPlaceholder.remove(); } else if (__pipOrigParent) { if (__pipOrigNext && __pipOrigNext.parentNode === __pipOrigParent) { __pipOrigParent.insertBefore(video, __pipOrigNext); } else { __pipOrigParent.appendChild(video); } } } finally { resetPiPState(); } }); return true; } catch (e) { log.debug('Document PiP failed, fallback to video PiP', e); } }
      if (document.pictureInPictureElement === video) return true; if (document.pictureInPictureEnabled && typeof video.requestPictureInPicture === 'function') { try { await video.requestPictureInPicture(); return true; } catch (e) { return false; } } if (typeof video.webkitSupportsPresentationMode === 'function' && video.webkitSupportsPresentationMode('picture-in-picture')) { try { video.webkitSetPresentationMode('picture-in-picture'); return true; } catch (e) { return false; } } return false;
    }
    async function exitPiP(preferredVideo = null) { if (__activeDocumentPiPWindow) { __activeDocumentPiPWindow.close(); return true; } if (document.pictureInPictureElement && document.exitPictureInPicture) { try { await document.exitPictureInPicture(); return true; } catch (_) {} } const candidates = []; if (preferredVideo) candidates.push(preferredVideo); const wk = findWebkitPiPVideo(); if (wk) candidates.push(wk); for (const v of candidates) { try { if (v && typeof v.webkitPresentationMode === 'string' && v.webkitPresentationMode === 'picture-in-picture' && typeof v.webkitSetPresentationMode === 'function') { v.webkitSetPresentationMode('inline'); return true; } } catch (_) {} } return false; }
    async function togglePiPFor(video) { if (!video || video.readyState < 2) return false; if (__activeDocumentPiPWindow || document.pictureInPictureElement === video) return exitPiP(video); if (document.pictureInPictureElement && document.exitPictureInPicture) { try { await document.exitPictureInPicture(); } catch (_) {} } return enterPiP(video); }

    function createZoomManager() {
      const stateMap = new WeakMap(); let rafId = null, activeVideo = null, isPanning = false, startX = 0, startY = 0; let pinchState = { active: false, initialDist: 0, initialScale: 1, lastCx: 0, lastCy: 0 }; const getSt = (v) => { let st = stateMap.get(v); if (!st) { st = { scale: 1, tx: 0, ty: 0, hasPanned: false, zoomed: false, origZIndex: '', origPosition: '' }; stateMap.set(v, st); } return st; };
      const update = (v) => { if (rafId) return; rafId = requestAnimationFrame(() => { rafId = null; const st = getSt(v); v.style.transition = isPanning || pinchState.active ? 'none' : 'transform 0.1s ease-out'; if (st.scale <= 1) { st.scale = 1; st.tx = 0; st.ty = 0; v.style.transform = ''; v.style.transformOrigin = ''; v.style.cursor = ''; if (st.zoomed) { v.style.zIndex = st.origZIndex; v.style.position = st.origPosition; st.zoomed = false; } } else { if (!st.zoomed) { st.origZIndex = v.style.zIndex; st.origPosition = v.style.position; st.zoomed = true; } v.style.transformOrigin = '0 0'; v.style.transform = `translate(${st.tx}px, ${st.ty}px) scale(${st.scale})`; v.style.cursor = isPanning ? 'grabbing' : 'grab'; v.style.zIndex = '2147483646'; if (window.getComputedStyle(v).position === 'static') { v.style.position = 'relative'; } } }); };
      const zoomTo = (v, newScale, clientX, clientY) => { const st = getSt(v), rect = v.getBoundingClientRect(), ix = (clientX - rect.left) / st.scale, iy = (clientY - rect.top) / st.scale; st.tx = clientX - (rect.left - st.tx) - ix * newScale; st.ty = clientY - (rect.top - st.ty) - iy * newScale; st.scale = newScale; update(v); }; const resetZoom = (v) => { if (v) { const st = getSt(v); st.scale = 1; update(v); } }; const isZoomed = (v) => { const st = stateMap.get(v); return st ? st.scale > 1 : false; }; const isZoomEnabled = () => !!window.__VSC_INTERNAL__?.Store?.get(P.APP_ZOOM_EN); const getTouchDist = (t) => Math.hypot(t[0].clientX - t[1].clientX, t[0].clientY - t[1].clientY); const getTouchCenter = (t) => ({ x: (t[0].clientX + t[1].clientX) / 2, y: (t[0].clientY + t[1].clientY) / 2 });
      function getTargetVideo(e) { const path = typeof e.composedPath === 'function' ? e.composedPath() : null; if (path) { for (const n of path) { if (n && n.tagName === 'VIDEO') return n; } } const cx = Number.isFinite(e.clientX) ? e.clientX : (e.touches && Number.isFinite(e.touches[0]?.clientX) ? e.touches[0].clientX : innerWidth * 0.5); const cy = Number.isFinite(e.clientY) ? e.clientY : (e.touches && Number.isFinite(e.touches[0]?.clientY) ? e.touches[0].clientHeight : innerHeight * 0.5); const el = document.elementFromPoint(cx, cy); let v = el?.tagName === 'VIDEO' ? el : el?.closest?.('video') || null; if (!v && window.__VSC_INTERNAL__?.App) { v = window.__VSC_INTERNAL__.App.getActiveVideo(); } return v; }
      onWin('wheel', e => { if (!e.altKey) return; const v = getTargetVideo(e); if (!v) return; e.preventDefault(); e.stopPropagation(); const delta = e.deltaY > 0 ? 0.9 : 1.1, st = getSt(v); let newScale = Math.min(Math.max(1, st.scale * delta), 10); if (newScale < 1.05) resetZoom(v); else zoomTo(v, newScale, e.clientX, e.clientY); }, { passive: false, capture: true }); onWin('mousedown', e => { if (!e.altKey) return; const v = getTargetVideo(e); if (!v) return; const st = getSt(v); if (st.scale > 1) { e.preventDefault(); e.stopPropagation(); activeVideo = v; isPanning = true; st.hasPanned = false; startX = e.clientX - st.tx; startY = e.clientY - st.ty; update(v); } }, { capture: true }); onWin('mousemove', e => { if (!isPanning || !activeVideo) return; e.preventDefault(); e.stopPropagation(); const st = getSt(activeVideo), dx = e.clientX - startX - st.tx, dy = e.clientY - startY - st.ty; if (Math.abs(dx) > 3 || Math.abs(dy) > 3) st.hasPanned = true; st.tx = e.clientX - startX; st.ty = e.clientY - startY; update(activeVideo); }, { capture: true }); onWin('mouseup', e => { if (isPanning) { if (activeVideo) { const st = getSt(activeVideo); if (st.hasPanned && e.cancelable) { e.preventDefault(); e.stopPropagation(); } update(activeVideo); } isPanning = false; activeVideo = null; } }, { capture: true }); onWin('dblclick', e => { if (!e.altKey) return; const v = getTargetVideo(e); if (!v) return; e.preventDefault(); e.stopPropagation(); const st = getSt(v); if (st.scale === 1) zoomTo(v, 2.5, e.clientX, e.clientY); else resetZoom(v); }, { capture: true }); onWin('touchstart', e => { if (CONFIG.IS_MOBILE && !isZoomEnabled()) return; const v = getTargetVideo(e); if (!v) return; const st = getSt(v); if (e.touches.length === 2) { if (e.cancelable) e.preventDefault(); activeVideo = v; pinchState.active = true; pinchState.initialDist = getTouchDist(e.touches); pinchState.initialScale = st.scale; const c = getTouchCenter(e.touches); pinchState.lastCx = c.x; pinchState.lastCy = c.y; } else if (e.touches.length === 1 && st.scale > 1) { activeVideo = v; isPanning = true; st.hasPanned = false; startX = e.touches[0].clientX - st.tx; startY = e.touches[0].clientY - st.ty; } }, { passive: false, capture: true }); onWin('touchmove', e => { if (!activeVideo) return; const st = getSt(activeVideo); if (pinchState.active && e.touches.length === 2) { if (e.cancelable) e.preventDefault(); const dist = getTouchDist(e.touches), center = getTouchCenter(e.touches); let newScale = pinchState.initialScale * (dist / Math.max(1, pinchState.initialDist)); newScale = Math.min(Math.max(1, newScale), 10); if (newScale < 1.05) { resetZoom(activeVideo); pinchState.active = false; } else { zoomTo(activeVideo, newScale, center.x, center.y); st.tx += center.x - pinchState.lastCx; st.ty += center.y - pinchState.lastCy; update(activeVideo); } pinchState.lastCx = center.x; pinchState.lastCy = center.y; } else if (isPanning && e.touches.length === 1) { if (e.cancelable) e.preventDefault(); const dx = e.touches[0].clientX - startX - st.tx, dy = e.touches[0].clientY - startY - st.ty; if (Math.abs(dx) > 3 || Math.abs(dy) > 3) st.hasPanned = true; st.tx = e.touches[0].clientX - startX; st.ty = e.touches[0].clientY - startY; update(activeVideo); } }, { passive: false, capture: true }); onWin('touchend', e => { if (!activeVideo) return; if (e.touches.length < 2) pinchState.active = false; if (e.touches.length === 0) { if (isPanning && getSt(activeVideo).hasPanned && e.cancelable) { e.preventDefault(); } isPanning = false; update(activeVideo); activeVideo = null; } }, { passive: false, capture: true }); return { resetZoom, zoomTo, isZoomed };
    }

    function createTargeting() {
      let stickyTarget = null; let stickyScore = -Infinity; let stickyUntil = 0;
      function pickFastActiveOnly(videos, lastUserPt, audioBoostOn) { const now = performance.now(); const vp = getViewportSnapshot(); let best = null, bestScore = -Infinity; const evalScore = (v) => { if (!v || v.readyState < 2) return; const r = getRectCached(v, now, 420); const area = r.width * r.height; const pip = isPiPActiveVideo(v); if (area < 160 * 120 && !pip) return; const cx = r.left + r.width * 0.5; const cy = r.top + r.height * 0.5; let s = 0; if (!v.paused && !v.ended) s += 6.0; if (v.currentTime > 0.2) s += 2.0; s += Math.log2(1 + area / 20000) * 1.1; const ptAge = Math.max(0, now - (lastUserPt.t || 0)); const userBias = Math.exp(-ptAge / 1800); const dx = cx - lastUserPt.x, dy = cy - lastUserPt.y; s += (2.0 * userBias) / (1 + (dx*dx + dy*dy) / 722500); const cdx = cx - vp.cx, cdy = cy - vp.cy; s += 0.7 / (1 + (cdx*cdx + cdy*cdy) / 810000); if (!v.muted && v.volume > 0.01) s += (audioBoostOn ? 2.2 : 1.2); if (pip) s += 3.0; if (s > bestScore) { bestScore = s; best = v; } }; for (const v of videos) { evalScore(v); } const activePip = getActivePiPVideo(); if (activePip && activePip.isConnected && !videos.has(activePip)) { evalScore(activePip); } if (stickyTarget && stickyTarget.isConnected && now < stickyUntil) { if (best && stickyTarget !== best && (bestScore < stickyScore + RUNTIME_GUARD.targeting.hysteresisMargin)) { return { target: stickyTarget }; } } stickyTarget = best; stickyScore = bestScore; stickyUntil = now + RUNTIME_GUARD.targeting.hysteresisMs; return { target: best }; }
    ...
    function createUtils() { return { clamp: VSC_CLAMP, h: (tag, props = {}, ...children) => { const el = (tag === 'svg' || props.ns === 'svg') ? document.createElementNS('http://www.w3.org/2000/svg', tag) : document.createElement(tag); for (const [k, v] of Object.entries(props)) { if (k.startsWith('on')) { el.addEventListener(k.slice(2).toLowerCase(), (e) => { if (k === 'onclick' && (tag === 'button' || tag === 'input')) e.stopPropagation(); v(e); }); } else if (k === 'style') { if (typeof v === 'string') el.style.cssText = v; else Object.assign(el.style, v); } else if (k === 'class') el.className = v; else if (v !== false && v != null && k !== 'ns') el.setAttribute(k, v); } children.flat().forEach(c => { if (c != null) el.append(typeof c === 'string' ? document.createTextNode(c) : c); }); return el; }, deepClone: (x) => (window.structuredClone ? structuredClone(x) : JSON.parse(JSON.stringify(x))), createLRU: (max = 384) => { const m = new Map(); return { get(k) { if (!m.has(k)) return undefined; const v = m.get(k); m.delete(k); m.set(k, v); return v; }, set(k, v) { if (m.has(k)) m.delete(k); m.set(k, v); if (m.size > max) m.delete(m.keys().next().value); } } } }; }

    function createFrameDriver() { let rafId = 0, timerId = 0, rvfcId = 0, lastVideo = null; function clear() { if (rafId) { cancelAnimationFrame(rafId); rafId = 0; } if (timerId) { clearTimeout(timerId); timerId = 0; } if (lastVideo && rvfcId && typeof lastVideo.cancelVideoFrameCallback === 'function') { try { lastVideo.cancelVideoFrameCallback(rvfcId); } catch (_) {} } rvfcId = 0; } function scheduleVideoFrame(nextVideo, cb, fallback = 'raf', fallbackMs = 90) { clear(); lastVideo = nextVideo || null; if (lastVideo && !lastVideo.paused && typeof lastVideo.requestVideoFrameCallback === 'function') { try { rvfcId = lastVideo.requestVideoFrameCallback((now, meta) => { rvfcId = 0; cb(now, meta); }); return; } catch (_) {} } if (fallback === 'timeout') { timerId = setTimeout(() => { timerId = 0; cb(performance.now(), null); }, fallbackMs); } else { rafId = requestAnimationFrame((t) => { rafId = 0; cb(t, null); }); } } return Object.freeze({ clear, scheduleVideoFrame }); }
    function patchInlineStyleImportant(el, patchObj) { const prev = new Map(); for (const [prop, value] of Object.entries(patchObj)) { prev.set(prop, { val: el.style.getPropertyValue(prop), prio: el.style.getPropertyPriority(prop) }); el.style.setProperty(prop, value, 'important'); } return function restore() { for (const [prop, info] of prev.entries()) { if (info.val) el.style.setProperty(prop, info.val, info.prio || ''); else el.style.removeProperty(prop); } }; }
    ...
    function composeBaseVideoParams(out, vUser) {
      ...
      const preGammaF = 1.0 + ((pB.gammaF ?? 1.0) - 1.0) * mix;
      ...
      out.gain = 1.0; out.gamma = preGammaF; out.contrast = 1.0; out.satF = 1.0; out.bright = preBright; out.temp = 0; out.mid = 0; out.toe = 0; out.shoulder = 0;
      out.sharp = Math.min(preSharp, 150); out.sharp2 = Math.min(preSharp2, 150); out.clarity = Math.min(preClarity, 150);
      const totalSharpN = (out.sharp + out.sharp2 * 0.4 + out.clarity * 0.18) / 110;
      if (totalSharpN > 0.3) { const dampF = 1 - (totalSharpN - 0.3) * 0.15; out.bright *= Math.max(0.85, dampF); }
      return out;
    }
    function applyShadowBandStack(out, shadowBandMask) { const mask = (Number(shadowBandMask) | 0) & 7; if (!mask) return out; const bandTable = [ { bit: SHADOW_BAND.OUTER, add: { toe: -1.0, mid: -0.010, bright: -0.5, gammaMul: 0.990, contrastMul: 1.015, satMul: 1.005 } }, { bit: SHADOW_BAND.MID, add: { toe: -1.8, mid: -0.015, bright: -1.0, gammaMul: 0.980, contrastMul: 1.025, satMul: 1.010 } }, { bit: SHADOW_BAND.DEEP, add: { toe: -3.0, mid: -0.005, bright: -0.5, gammaMul: 0.985, contrastMul: 1.020, satMul: 1.000 } } ]; let toeAdd = 0, midAdd = 0, brightAdd = 0; let gammaMul = 1, contrastMul = 1, satMul = 1; for (const band of bandTable) { if (!ShadowMask.has(mask, band.bit)) continue; toeAdd += band.add.toe; midAdd += band.add.mid; brightAdd += band.add.bright; gammaMul *= band.add.gammaMul; contrastMul *= band.add.contrastMul; satMul *= band.add.satMul; } out.toe = (out.toe || 0) + toeAdd; out.mid = (out.mid || 0) + midAdd; out.bright = (out.bright || 0) + brightAdd; out.gamma = (out.gamma || 1) * gammaMul; out.contrast = (out.contrast || 1) * contrastMul; out.satF = (out.satF || 1) * satMul; return out; }
    function applyBrightStepStack(out, brightStepLevel) { const lvl = Math.max(0, Math.min(3, Math.round(Number(brightStepLevel) || 0))); if (!lvl) return out; const table = { 1: { brightAdd: 1.8, gammaMul: 1.02, contrastMul: 0.995 }, 2: { brightAdd: 3.8, gammaMul: 1.05, contrastMul: 0.990 }, 3: { brightAdd: 6.0, gammaMul: 1.08, contrastMul: 0.985 } }; const s = table[lvl]; out.bright = (out.bright || 0) + s.brightAdd; out.gamma = (out.gamma || 1) * s.gammaMul; out.contrast = (out.contrast || 1) * s.contrastMul; return out; }
    function composeVideoParamsInto(out, vUser, isHdr) { composeBaseVideoParams(out, vUser); applyShadowBandStack(out, vUser.shadowBandMask); applyBrightStepStack(out, vUser.brightStepLevel); const autoMods = window.__VSC_INTERNAL__?.AutoScene?.getMods?.() || { br: 1.0, ct: 1.0, sat: 1.0 }; if (autoMods) { out.gain = (out.gain || 1.0) * autoMods.br; out.contrast = (out.contrast || 1.0) * autoMods.ct; out.satF = (out.satF || 1.0) * autoMods.sat; } if (isHdr) { out.bright = (out.bright || 0) * 0.65; out.contrast = 1.0 + ((out.contrast || 1.0) - 1.0) * 0.75; out.satF = 1.0 + ((out.satF || 1.0) - 1.0) * 0.85; } return out; }
    function projectVValsForWebGL(vVals) { const out = { ...vVals }; const clamp = VSC_CLAMP; const sharp = Number(out.sharp || 0), sharp2 = Number(out.sharp2 || 0), clarity = Number(out.clarity || 0); let totalSharp = sharp + sharp2 * 0.40 + clarity * 0.18; out.sharp = clamp(totalSharp, 0, 150); out.bright = Number(out.bright || 0); out.contrast = Number(out.contrast || 1); out.gamma = Number(out.gamma || 1); out.sharp2 = 0; out.clarity = 0; out.toe = Number(out.toe || 0); out.mid = Number(out.mid || 0); out.shoulder = Number(out.shoulder || 0); return out; }
    ...
    function createVideoParamsMemo(Store, P, Utils) { let lastKey = ''; let lastSvg = null; let lastWebgl = null; const sigVideo = (vf) => [ vf.presetS, vf.presetB, Number(vf.presetMix).toFixed(3), (vf.shadowBandMask|0), (vf.brightStepLevel|0), ].join('|'); return { get(vfUser, rMode, activeTarget) { const w = activeTarget ? (activeTarget.videoWidth || 0) : 0; const h = activeTarget ? (activeTarget.videoHeight || 0) : 0; const hdr = VSC_MEDIA.isHdr() ? 1 : 0; const autoMods = window.__VSC_INTERNAL__?.AutoScene?.getMods?.() || { br: 1.0, ct: 1.0, sat: 1.0 }; const autoKey = `${autoMods.br.toFixed(3)}|${autoMods.ct.toFixed(3)}|${autoMods.sat.toFixed(3)}`; const key = `${sigVideo(vfUser)}|${rMode}|${w}x${h}|hdr:${hdr}|auto:${autoKey}`; if (key === lastKey && lastSvg && lastWebgl) { return rMode === 'webgl' ? lastWebgl : lastSvg; } const base = {}; composeVideoParamsInto(base, vfUser, hdr === 1); base.sharp = Math.min(Number(base.sharp || 0), 36); const webgl = projectVValsForWebGL(base); lastSvg = base; lastWebgl = webgl; lastKey = key; return rMode === 'webgl' ? lastWebgl : lastSvg; } }; }
    ...
    function createAudio(sm) {
      let ctx, compressor, limiter, wetInGain, dryOut, wetOut, masterOut, hpf, clipper, target = null, currentSrc = null; let kWeightShelf, kWeightPass, analyser; let dataArray; let srcMap = new WeakMap(); let makeupDbEma = 0; let switchTimer = 0, switchTok = 0; let gestureHooked = false; let loopTok = 0; const VSC_AUD_HPF_HZ = 28; const VSC_AUD_HPF_Q = 0.707; const VSC_AUD_CLIP_KNEE = 0.985; const VSC_AUD_CLIP_DRIVE = 6.0; let __vscClipCurve = null; function getSoftClipCurve() { if (__vscClipCurve) return __vscClipCurve; const n = 4096; const knee = VSC_AUD_CLIP_KNEE; const drive = VSC_AUD_CLIP_DRIVE; const curve = new Float32Array(n); const tanhD = Math.tanh(drive); for (let i = 0; i < n; i++) { const x = (i / (n - 1)) * 2 - 1; const ax = Math.abs(x); let y; if (ax <= knee) { y = x; } else { const t = (ax - knee) / Math.max(1e-6, (1 - knee)); const s = Math.tanh(drive * t) / tanhD; y = Math.sign(x) * (knee + (1 - knee) * s); } curve[i] = y; } __vscClipCurve = curve; return curve; } const onGesture = async () => { try { if (ctx && ctx.state === 'suspended') { await ctx.resume(); } if (ctx && ctx.state === 'running' && gestureHooked) { window.removeEventListener('pointerdown', onGesture, true); window.removeEventListener('keydown', onGesture, true); gestureHooked = false; } } catch (_) {} }; const ensureGestureResumeHook = () => { if (gestureHooked) return; gestureHooked = true; onWin('pointerdown', onGesture, { passive: true, capture: true }); onWin('keydown', onGesture, { passive: true, capture: true }); }; const clamp = VSC_CLAMP; const VSC_AUDIO_AUTO_MAKEUP = true; function runAudioLoop(tok) { if (tok !== loopTok || !ctx) return; const en = !!(sm.get(P.A_EN) && sm.get(P.APP_ACT)); const actuallyEnabled = en && currentSrc; if (VSC_AUDIO_AUTO_MAKEUP && actuallyEnabled && analyser) { analyser.getFloatTimeDomainData(dataArray); let sumSquare = 0; for(let i = 0; i < dataArray.length; i++) { sumSquare += dataArray[i] * dataArray[i]; } const rms = Math.sqrt(sumSquare / dataArray.length); const db = rms > 1e-6 ? 20 * Math.log10(rms) : -100; let redDb = 0; try { const r = compressor?.reduction; redDb = (typeof r === 'number') ? r : (r && typeof r.value === 'number') ? r.value : 0; } catch (_) {} if (!Number.isFinite(redDb)) redDb = 0; const redPos = clamp(-redDb, 0, 18); let gateMult = 1.0; if (db < -45) { gateMult = 0.0; } else if (db < -40) { gateMult = (db - (-45)) / 5.0; } const makeupDbTarget = clamp(Math.max(0, redPos - 2.0) * 0.22, 0, 2.8) * gateMult; const isAttack = makeupDbTarget < makeupDbEma; const alpha = isAttack ? 0.35 : 0.015; makeupDbEma += (makeupDbTarget - makeupDbEma) * alpha; } else { makeupDbEma += (0 - makeupDbEma) * 0.1; } const boostDb = Number(sm.get(P.A_BST) || 0); const userBoost = Math.pow(10, boostDb / 20); const makeup = Math.pow(10, makeupDbEma / 20); if (wetInGain) { const finalGain = actuallyEnabled ? (userBoost * makeup) : 1.0; try { wetInGain.gain.setTargetAtTime(finalGain, ctx.currentTime, 0.05); } catch (_) { wetInGain.gain.value = finalGain; } } setTimeout(() => runAudioLoop(tok), 70); }
      const ensureCtx = () => { if (ctx && ctx.state === 'closed') { ctx = null; compressor = null; limiter = null; wetInGain = null; dryOut = null; wetOut = null; masterOut = null; hpf = null; clipper = null; currentSrc = null; target = null; kWeightShelf = null; kWeightPass = null; analyser = null; dataArray = null; srcMap = new WeakMap(); } if (ctx) return true; const AC = window.AudioContext || window.webkitAudioContext; if (!AC) return false; try { ctx = new AC({ latencyHint: 'playback' }); } catch (_) { ctx = new AC(); } ensureGestureResumeHook(); compressor = ctx.createDynamicsCompressor(); compressor.threshold.value = -22; compressor.knee.value = 24; compressor.ratio.value = 2.6; compressor.attack.value = 0.006; compressor.release.value = 0.25; limiter = ctx.createDynamicsCompressor(); limiter.threshold.value = -1.2; limiter.knee.value = 0.0; limiter.ratio.value = 20.0; limiter.attack.value = 0.0015; limiter.release.value = 0.09; hpf = ctx.createBiquadFilter(); hpf.type = 'highpass'; hpf.frequency.value = VSC_AUD_HPF_HZ; hpf.Q.value = VSC_AUD_HPF_Q; clipper = ctx.createWaveShaper(); clipper.curve = getSoftClipCurve(); try { clipper.oversample = '4x'; } catch (_) {} analyser = ctx.createAnalyser(); analyser.fftSize = 2048; dataArray = new Float32Array(analyser.fftSize); kWeightShelf = ctx.createBiquadFilter(); kWeightShelf.type = 'highshelf'; kWeightShelf.frequency.value = 1500; kWeightShelf.gain.value = 4.0; kWeightPass = ctx.createBiquadFilter(); kWeightPass.type = 'highpass'; kWeightPass.frequency.value = 38; kWeightPass.Q.value = 0.5; dryOut = ctx.createGain(); wetOut = ctx.createGain(); wetInGain = ctx.createGain(); masterOut = ctx.createGain(); dryOut.connect(masterOut); wetOut.connect(masterOut); hpf.connect(compressor); hpf.connect(kWeightShelf); compressor.connect(wetInGain); wetInGain.connect(limiter); limiter.connect(clipper); clipper.connect(wetOut); kWeightShelf.connect(kWeightPass); kWeightPass.connect(analyser); masterOut.connect(ctx.destination); return true; };
      const rampGainsSafe = (dryTarget, wetTarget, tc = 0.015) => { if (!ctx) return; const t = ctx.currentTime; try { dryOut.gain.cancelScheduledValues(t); wetOut.gain.cancelScheduledValues(t); dryOut.gain.setTargetAtTime(dryTarget, t, tc); wetOut.gain.setTargetAtTime(wetTarget, t, tc); } catch (_) { dryOut.gain.value = dryTarget; wetOut.gain.value = wetTarget; } }; const fadeOutThen = (fn) => { if (!ctx) { fn(); return; } const tok = ++switchTok; clearTimeout(switchTimer); const t = ctx.currentTime; try { masterOut.gain.cancelScheduledValues(t); masterOut.gain.setValueAtTime(masterOut.gain.value, t); masterOut.gain.linearRampToValueAtTime(0, t + 0.04); } catch (_) { masterOut.gain.value = 0; } switchTimer = setTimeout(() => { if (tok !== switchTok) return; makeupDbEma = 0; try { fn(); } catch (_) {} if (ctx) { const t2 = ctx.currentTime; try { masterOut.gain.cancelScheduledValues(t2); masterOut.gain.setValueAtTime(0, t2); masterOut.gain.linearRampToValueAtTime(1, t2 + 0.04); } catch (_) { masterOut.gain.value = 1; } } }, 60); }; const disconnectAll = () => { if (currentSrc) { try { currentSrc.disconnect(); } catch (_) {} } currentSrc = null; target = null; }; const updateMix = () => { if (!ctx) return; const en = !!(sm.get(P.A_EN) && sm.get(P.APP_ACT)); const isHooked = !!currentSrc; const actuallyEnabled = en && isHooked; const dryTarget = actuallyEnabled ? 0 : 1; const wetTarget = actuallyEnabled ? 1 : 0; rampGainsSafe(dryTarget, wetTarget, 0.015); loopTok++; if (actuallyEnabled) { runAudioLoop(loopTok); } }; async function destroy() { try { fadeOutThen(() => disconnectAll()); } catch (_) {} try { if (gestureHooked) { window.removeEventListener('pointerdown', onGesture, true); window.removeEventListener('keydown', onGesture, true); gestureHooked = false; } } catch (_) {} loopTok++; try { if (ctx && ctx.state !== 'closed') await ctx.close(); } catch (_) {} ctx = null; compressor = null; limiter = null; wetInGain = null; dryOut = null; wetOut = null; masterOut = null; hpf = null; clipper = null; currentSrc = null; target = null; kWeightShelf = null; kWeightPass = null; analyser = null; dataArray = null; makeupDbEma = 0; switchTok++; srcMap = new WeakMap(); }
      return { setTarget: (v) => { const enabled = !!(sm.get(P.A_EN) && sm.get(P.APP_ACT)); const st = v ? getVState(v) : null; if (st && st.audioFailUntil > performance.now()) { if (v !== target) { fadeOutThen(() => { disconnectAll(); target = v; }); } updateMix(); return; } if (!ensureCtx()) return; if (v === target) { updateMix(); return; } fadeOutThen(() => { disconnectAll(); target = v; if (!v) { updateMix(); return; } try { let s = srcMap.get(v); if (!s) { s = ctx.createMediaElementSource(v); srcMap.set(v, s); } s.connect(dryOut); s.connect(hpf || compressor); currentSrc = s; } catch (_) { if (st && VSC_DEFENSE.audioCooldown) st.audioFailUntil = performance.now() + RUNTIME_GUARD.audio.createSourceCooldownMs; disconnectAll(); } updateMix(); }); }, update: updateMix, hasCtx: () => !!ctx, isHooked: () => !!currentSrc, destroy };
    }

    function createAutoSceneManager(Store, P, Scheduler) {
      const LUMA = { r: 0.2126, g: 0.7152, b: 0.0722 }; const clamp = VSC_CLAMP; const approach = (cur, tgt, a, dead=0.002) => { const d = tgt - cur; return Math.abs(d) < dead ? tgt : cur + d * a; };
      const AUTO = { running: false, canvasW: 64, canvasH: 36, cur: { br: 1.0, ct: 1.0, sat: 1.0 }, tgt: { br: 1.0, ct: 1.0, sat: 1.0 }, lastSig: null, cutHist: [], motionEma: 0, motionAlpha: 0.30, motionThresh: 0.0075, motionFrames: 0, motionMinFrames: 5, statsEma: null, statsAlpha: 0.12, drmBlocked: false, blockUntilMs: 0, tBoostUntil: 0, tBoostStart: 0, boostMs: 800, minBoostEarlyMs: 700, fpsHist: [], minFps: 1, maxFps: 6, curFps: 1, _lumaN: 0, _lumaA: null, _lumaB: null, _lumaFlip: 0 };
      ...
      function computeStatsAndMotion(AUTO, img, sw, sh) { const data = img.data; const n = sw * sh; ensureLumaBuffers(AUTO, n); const cur = (AUTO._lumaFlip === 0) ? AUTO._lumaA : AUTO._lumaB; const prev = (AUTO._lumaFlip === 0) ? AUTO._lumaB : AUTO._lumaA; let sum = 0, sum2 = 0, sumEdge = 0; let diffSum = 0; let sumChroma = 0; for (let y = 0; y < sh; y += 2) { let row = y * sw; let idx = (y * sw) * 4; for (let x = 0; x < sw; x += 2) { const r = data[idx], g = data[idx + 1], b = data[idx + 2]; const l = (r * 0.2126 + g * 0.7152 + b * 0.0722) | 0; const max = r > g ? (r > b ? r : b) : (g > b ? g : b); const min = r < g ? (r < b ? r : b) : (g < b ? g : b); sumChroma += (max - min); const p = row + x; cur[p] = l; sum += l; sum2 += l * l; if (x + 2 < sw) { const r2 = data[idx + 8], g2 = data[idx + 9], b2 = data[idx + 10]; const l2 = (r2 * 0.2126 + g2 * 0.7152 + b2 * 0.0722) | 0; sumEdge += Math.abs(l2 - l); } if (y + 2 < sh) { const idxD = idx + (sw * 2 * 4); const r3 = data[idxD], g3 = data[idxD + 1], b3 = data[idxD + 2]; const l3 = (r3 * 0.2126 + g3 * 0.7152 + b3 * 0.0722) | 0; sumEdge += Math.abs(l3 - l); } diffSum += Math.abs(l - prev[p]); idx += 8; } } AUTO._lumaFlip ^= 1; const samples = Math.max(1, ((sw + 1) >> 1) * ((sh + 1) >> 1)); const mean = sum / samples; const var_ = (sum2 / samples) - mean * mean; const std = Math.sqrt(Math.max(0, var_)); const edge = sumEdge / samples; const motion = diffSum / samples; const bright = mean / 255; const contrast = std / 64; const chroma = (sumChroma / samples) / 255; return { bright, contrast, chroma, edge, motion }; }
...
// ⭐ v170.8.0 패치: Alpha 전파 수정 + 모바일 샤프닝 적응형 분기 + 톤 캐시/키 최적화
    function createFiltersVideoOnly(Utils, config) {
      ...
      function getToneTableCached(steps, toeN, shoulderN, midN, gain, contrast, brightOffset, gamma) {
        const key = steps + '|' + toeN + '|' + shoulderN + '|' + midN + '|' + gain + '|' + contrast.toFixed(2) + '|' + brightOffset.toFixed(2) + '|' + gamma.toFixed(2);
      ...
      function buildSvg(root) {
        ...
        // ✨ 패치 1-4: 알파 강제 패치 (edgeGate, hlKeep, invGate)
        const edgeGate = mkXfer({ in: 'rngBlur', result: 'edgeGate' }, { type: 'table', tableValues: '0 1' }, true);
        const hlKeep   = mkXfer({ in: 'lumBase', result: 'hlKeep' }, { type: 'table', tableValues: '1 1' }, true);
        ...
        const invGate = mkXfer({ in: 'gate', result: 'invGate' }, { type: 'table', tableValues: '1 0' }, true);
        ...
        return { fidLite, fidMid, fidFull, lite: { toneFuncs: Array.from(toneLite.children), tmpFuncs: Array.from(tmpLite.children), sat: satLite }, mid: { toneFuncs: Array.from(toneMid.children), tmpFuncs: Array.from(tmpMid.children), sat: satMid, convMid }, full: { toneFuncs: Array.from(tone.children), tmpFuncs: Array.from(tmp.children), sat, detail: { b1, sh1, b2, sh2, bc, cl }, casLike: { rngMax, rngMin, rngBlur, edgeGateFuncs: Array.from(edgeGate.children).filter(n => n.tagName !== 'feFuncA'), hlKeepFuncs: Array.from(hlKeep.children).filter(n => n.tagName !== 'feFuncA'), sharpDesat, sharpBiased } }, st: { lastKey: '', toneKey: '', tempKey: '', satKey: '', detailKey: '', midConvKey: '', casKey: '', morphKey: '', edgeGateTable: '', hlKeepTable: '', __b1: '', __sh1k2: '', __sh1k3: '', __b2: '', __sh2k2: '', __sh2k3: '', __bc: '', __clk2: '', __clk3: '', __rngMaxR: '', __rngMinR: '', __rngBlur: '', __sharpDesatSat: '', __sharpBiasK2: '', __sharpBiasK3: '' } };
      }
      ...
      function prepare(video, s) {
        ...
        if (ctx.st.lastKey !== key) {
          ...
          const tk = steps + '|' + toeQ + '|' + shQ + '|' + midQ + '|' + gainQ + '|' + con.toFixed(2) + '|' + brOff.toFixed(2) + '|' + gamma.toFixed(2);
          ...
          const ck = sharpTotal.toFixed(2) + '|' + (s.bright || 0).toFixed(2) + '|' + (s.gamma || 1).toFixed(3) + '|' + (s.contrast || 1).toFixed(3) + '|' + vw + '|' + vh + '|' + morphRadius;
          ...
          queueMicrotask(() => {
            ...
            if (filterGrade === 'M' && detailOn) {
              ...
              if (st.midConvKey !== mk) {
                st.midConvKey = mk; const midSharpMul = config.IS_MOBILE ? 0.45 : 0.30;
                ...
                if (totalS > 0.008) {
                  ...
                  ctx.mid.convMid.setAttribute('kernelMatrix', `0 ${edge.toFixed(4)} 0  ${edge.toFixed(4)} ${center.toFixed(4)} ${edge.toFixed(4)}  0 ${edge.toFixed(4)} 0`);
                } else {
            ...
            if (filterGrade === 'F' && detailOn) {
              const fn = ctx.full;
              if (st.detailKey !== dk) {
                st.detailKey = dk; const sc = (x) => x * x * (3 - 2 * x);
                const fullSharpMul = config.IS_MOBILE ? 0.65 : 0.55;

                const v1 = (s.sharp || 0) / 50 * fullSharpMul; const kC = sc(Math.min(1, v1)) * 2.2;
                setAttr(fn.detail.b1, 'stdDeviation', v1 > 0 ? (0.65 - sc(Math.min(1, v1)) * 0.2).toFixed(2) : '0', st, '__b1');
                setAttr(fn.detail.sh1, 'k2', (1 + kC).toFixed(3), st, '__sh1k2'); setAttr(fn.detail.sh1, 'k3', (-kC).toFixed(3), st, '__sh1k3');

                const v2 = (s.sharp2 || 0) / 50 * fullSharpMul; const kF = sc(Math.min(1, v2)) * 4.8;
                setAttr(fn.detail.b2, 'stdDeviation', v2 > 0 ? '0.25' : '0', st, '__b2');
                setAttr(fn.detail.sh2, 'k2', (1 + kF).toFixed(3), st, '__sh2k2'); setAttr(fn.detail.sh2, 'k3', (-kF).toFixed(3), st, '__sh2k3');

                const clVal = (s.clarity || 0) / 50 * fullSharpMul; const clStd = clVal > 0 ? (0.85 + hiResN * 0.55).toFixed(2) : '0'; const clGain = (1 + clVal * (1.15 + hiResN * 0.55));
                setAttr(fn.detail.bc, 'stdDeviation', clStd, st, '__bc');
                setAttr(fn.detail.cl, 'k2', clGain.toFixed(3), st, '__clk2'); setAttr(fn.detail.cl, 'k3', (-(clGain - 1)).toFixed(3), st, '__clk3');
              }

              if (st.casKey !== ck) {
                st.casKey = ck;
                if (morphRadius === 0) {
                  // ✨ 패치 2-2: Bypass 시 신호 통과 보장 (1 1)
                  const morphKey = '0'; if (st.morphKey !== morphKey) { st.morphKey = morphKey; setAttr(fn.casLike.rngMax, 'radius', '0', st, '__rngMaxR'); setAttr(fn.casLike.rngMin, 'radius', '0', st, '__rngMinR'); }
                  if (st.__rngBlur !== '0') { st.__rngBlur = '0'; fn.casLike.rngBlur.setAttribute('stdDeviation', '0'); }
                  if (st.edgeGateTable !== '1 1') { st.edgeGateTable = '1 1'; for (const f of fn.casLike.edgeGateFuncs) f.setAttribute('tableValues', '1 1'); }
                  if (st.hlKeepTable !== '1 1') { st.hlKeepTable = '1 1'; for (const f of fn.casLike.hlKeepFuncs) f.setAttribute('tableValues', '1 1'); }
                  if (st.__sharpBiasK2 !== '1' || st.__sharpBiasK3 !== '0') { st.__sharpBiasK2 = '1'; st.__sharpBiasK3 = '0'; fn.casLike.sharpBiased.setAttribute('k2', '1'); fn.casLike.sharpBiased.setAttribute('k3', '0'); }
                  if (st.__sharpDesatSat !== '1') { st.__sharpDesatSat = '1'; fn.casLike.sharpDesat.setAttribute('values', '1'); }
                } else {
                  const morphKey = String(morphRadius); if (st.morphKey !== morphKey) { st.morphKey = morphKey; setAttr(fn.casLike.rngMax, 'radius', morphKey, st, '__rngMaxR'); setAttr(fn.casLike.rngMin, 'radius', morphKey, st, '__rngMinR'); }
                  const rbk = rngBlurStd.toFixed(2); if (st.__rngBlur !== rbk) { st.__rngBlur = rbk; fn.casLike.rngBlur.setAttribute('stdDeviation', rbk); }
                  if (st.edgeGateTable !== edgeGateTable) { st.edgeGateTable = edgeGateTable; for (const f of fn.casLike.edgeGateFuncs) f.setAttribute('tableValues', edgeGateTable); }
                  if (st.hlKeepTable !== hlKeepTable) { st.hlKeepTable = hlKeepTable; for (const f of fn.casLike.hlKeepFuncs) f.setAttribute('tableValues', hlKeepTable); }
                  const ds = desatSat.toFixed(2); if (st.__sharpDesatSat !== ds) { st.__sharpDesatSat = ds; fn.casLike.sharpDesat.setAttribute('values', ds); }
                  const k2 = chromaKeep.toFixed(3), k3 = (1 - chromaKeep).toFixed(3); if (st.__sharpBiasK2 !== k2 || st.__sharpBiasK3 !== k3) { st.__sharpBiasK2 = k2; st.__sharpBiasK3 = k3; fn.casLike.sharpBiased.setAttribute('k2', k2); fn.casLike.sharpBiased.setAttribute('k3', k3); }
                }
              }
            }
          });
      ...
      return { prepareCached: (video, s) => { try { return prepare(video, s); } catch (e) { log.warn('filter prepare failed:', e); return null; } }, applyUrl: (el, url) => { if (!el) return; const st = getVState(el); if (!url) { if (st.applied) { queueMicrotask(() => { el.style.removeProperty('filter'); el.style.removeProperty('-webkit-filter'); }); st.applied = false; st.lastFilterUrl = null; } return; } if (st.lastFilterUrl === url) return; queueMicrotask(() => { el.style.setProperty('filter', url, 'important'); el.style.setProperty('-webkit-filter', url, 'important'); }); st.applied = true; st.lastFilterUrl = url; }, clear: (el) => { if (!el) return; const st = getVState(el); if (!st.applied) return; queueMicrotask(() => { el.style.removeProperty('filter'); el.style.removeProperty('-webkit-filter'); }); st.applied = false; st.lastFilterUrl = null; } };
    }
    ...
    function createFiltersWebGL(Utils) {
      ...
      function buildToneLUT256(toe, mid, shoulder) {
        const clamp = VSC_CLAMP; const steps = 256; const out = new Uint8Array(steps * 4);
        const t = clamp(toe / 14, -1, 1); const s = clamp(shoulder / 16, -1, 1); const m = clamp(mid, -1, 1);
        const smoothstep = (a,b,x)=>{ x = clamp((x-a)/(b-a),0,1); return x*x*(3-2*x); };
        let prev = 0;
        for (let i = 0; i < steps; i++) {
          let x = i / 255; x = clamp(x + m * 0.06 * (4 * x * (1 - x)), 0, 1);
          if (t !== 0) { const w = smoothstep(0.0, 0.35, x); x = clamp(x + t * 0.08 * (1 - w), 0, 1); }
          if (s !== 0) { const w = smoothstep(0.85, 1.0, x); x = clamp(x - s * 0.08 * w, 0, 1); }
          if (x < prev) x = prev; prev = x;
...
void main() {
  vec3 color = TEX(uVideoTex, vTexCoord).rgb; color *= uRGBGain; color += (uParams2.x / 1000.0); color = (color - 0.5) * uParams.y + 0.5; color *= uParams.x;
  ...
  if (uParams.w != 1.0) color = pow(max(color, vec3(0.0)), vec3(1.0 / uParams.w)); color = softClip(color, 0.18);
...
void main() {
  ...
  outColor = vec4(clamp(color, 0.0, 1.0), 1.0);
      ...
      class WebGLPipeline {
        // ✨ 패치 6-1: dropRateEma 구문 오류 수정 ( : 0 )
        constructor() { this.canvas = null; this.gl = null; this.activeProgramKind = ''; this.videoTexture = null; this.video = null; this.active = false; this.vVals = null; this.originalParent = null; this.restoreVideoStyle = null; this.renderDriver = createFrameDriver(); this.disabledUntil = 0; this._texW = 0; this._texH = 0; this._loopToken = 0; this._loopRunning = false; this._isGL2 = false; this._qMon = { lastT: 0, lastDropped: 0, dropRateEma: 0 }; this._styleDirty = true; this._styleObs = null; this._lastStyleSyncT = 0; this._parentStylePatched = false; this._parentPrevPosition = ''; this._patchedParent = null; this.toneTexture = null; this._toneKey = ''; this._outputReady = false; this._onContextLost = (e) => { e.preventDefault(); const now = performance.now(); this.disabledUntil = now + 3000; this.active = false; this._loopToken++; this._loopRunning = false; try { this.restoreVideoStyle?.(); } catch (_) {} this.restoreVideoStyle = null; try { if (this.canvas) this.canvas.style.opacity = '0'; } catch (_) {} try { const st = this.video ? getVState(this.video) : null; if (st && VSC_DEFENSE.webglCooldown) st.webglDisabledUntil = now + 5000; } catch (_) {} try { window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} }; this._onContextRestored = () => { try { this.disposeGLResources({ keepCanvasListeners: true }); if (this.initGLResourcesOnExistingCanvas()) { if (this.video) { this.active = true; this.startRenderLoop(); } } else { this.disabledUntil = performance.now() + 5000; } } catch (_) { this.disabledUntil = performance.now() + 5000; } }; this._lastRenderedTime = -1; this._paramsDirty = true; }
        ...
        attachToVideo(video) {
          ...
          this._styleDirty = true; this.active = true;
          // ✨ 패치 1-5: WebGL 루프 부활 리스너
        ...
        updateParams(vVals) { if (this.vVals !== vVals) { this._paramsDirty = true; } this.vVals = vVals; }
        syncCanvasPresentationFromVideo(video, now) { if (!this.canvas || !video) return; if (!this._styleDirty && (now - this._lastStyleSyncT) < 250) return; const vs = window.getComputedStyle(video), cs = this.canvas.style; if (cs.objectFit !== vs.objectFit) cs.objectFit = vs.objectFit || 'contain'; if (cs.objectPosition !== vs.objectPosition) cs.objectPosition = vs.objectPosition; const tr = vs.transform, tro = vs.transformOrigin; const nextTr = (tr && tr !== 'none') ? tr : ''; if (cs.transform !== nextTr) cs.transform = nextTr; if (cs.transformOrigin !== tro) cs.transformOrigin = tro; if (cs.borderRadius !== vs.borderRadius) cs.borderRadius = vs.borderRadius || ''; if (cs.clipPath !== vs.clipPath) cs.clipPath = vs.clipPath || ''; if (cs.webkitClipPath !== vs.webkitClipPath) cs.webkitClipPath = vs.webkitClipPath || ''; if (cs.mixBlendMode !== vs.mixBlendMode) cs.mixBlendMode = vs.mixBlendMode || ''; if (cs.isolation !== vs.isolation) cs.isolation = vs.isolation || ''; const vz = vs.zIndex; let zi = '1'; if (vz && vz !== 'auto') { const n = parseInt(vz, 10); zi = Number.isFinite(n) ? String(n + 1) : '1'; } if (cs.zIndex !== zi) cs.zIndex = zi; this._styleDirty = false; this._lastStyleSyncT = now; }
        _updatePlaybackQuality(now) { const v = this.video; if (!v || typeof v.getVideoPlaybackQuality !== 'function') return; if (now - this._qMon.lastT < 1000) return; try { const q = v.getVideoPlaybackQuality(), dropped = q.droppedVideoFrames || 0; if (this._qMon.lastT > 0) { const dd = Math.max(0, dropped - this._qMon.lastDropped); this._qMon.dropRateEma = this._qMon.dropRateEma ? (this._qMon.dropRateEma * 0.8 + dd * 0.2) : dd; } this._qMon.lastDropped = dropped; this._qMon.lastT = now; if (this._qMon.dropRateEma > 2.5) { const st = getVState(v); if (VSC_DEFENSE.webglCooldown) st.webglDisabledUntil = now + 8000; try { window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} } } catch (_) {} }
        render() { if (!this.active || !this.gl || !this.video || !this.vVals) return; const gl = this.gl, video = this.video; const now = performance.now(); if (now < this.disabledUntil) return; const st = getVState(video); if (st.webglDisabledUntil && now < st.webglDisabledUntil) return; if (video.readyState < 2 || video.videoWidth === 0 || video.videoHeight === 0) return; const ct = video.currentTime; if (this._lastRenderedTime === ct && !this._paramsDirty) return; this._lastRenderedTime = ct; this._paramsDirty = false; if (this.canvas.parentNode !== video.parentNode && video.parentNode) { this.originalParent = video.parentNode; const p = video.parentNode; if (video.nextSibling) p.insertBefore(this.canvas, video.nextSibling); else p.appendChild(this.canvas); } this.syncCanvasPresentationFromVideo(video, now); this._updatePlaybackQuality(now); const w = video.videoWidth, h = video.videoHeight; const sharpNorm = (this.vVals.sharp || 0) / 50.0; const useSharpen = sharpNorm > 0; const kind = useSharpen ? 'sharp' : 'color'; const H = useSharpen ? this.handles_sharp : this.handles_color; let programChanged = false; if (this.activeProgramKind !== kind) { this.activeProgramKind = kind; programChanged = true; gl.useProgram(H.program); gl.bindBuffer(gl.ARRAY_BUFFER, this.vBuf); gl.enableVertexAttribArray(H.aPosition); gl.vertexAttribPointer(H.aPosition, 2, gl.FLOAT, false, 0, 0); gl.bindBuffer(gl.ARRAY_BUFFER, this.tBuf); gl.enableVertexAttribArray(H.aTexCoord); gl.vertexAttribPointer(H.aTexCoord, 2, gl.FLOAT, false, 0, 0); } const resized = (this.canvas.width !== w || this.canvas.height !== h); if (resized) { this.canvas.width = w; this.canvas.height = h; gl.viewport(0, 0, w, h); } if ((resized || programChanged) && H.uResolution) gl.uniform2f(H.uResolution, w, h); const { rs, gs, bs } = tempToRgbGain(this.vVals.temp); if (H.uParams) gl.uniform4f(H.uParams, this.vVals.gain || 1.0, this.vVals.contrast || 1.0, this.vVals.satF || 1.0, this.vVals.gamma || 1.0); const isHdr = VSC_MEDIA.isHdr(); const hiReduce = isHdr ? 0.82 : 0.88; if (H.uParams2) gl.uniform4f(H.uParams2, this.vVals.bright || 0.0, useSharpen ? sharpNorm : 0.0, 0.0, hiReduce); if (H.uRGBGain) gl.uniform3f(H.uRGBGain, rs, gs, bs); const toe = this.vVals.toe || 0; const mid = this.vVals.mid || 0; const shoulder = this.vVals.shoulder || 0; const tq = (n, q) => (Math.round(n / q) * q).toFixed(3); const toneKey = tq(toe,0.2) + '|' + tq(mid,0.02) + '|' + tq(shoulder,0.2); if (this._toneKey !== toneKey && this.toneTexture) { this._toneKey = toneKey; const lut = buildToneLUT256(toe, mid, shoulder); gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, this.toneTexture); gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, 256, 1, gl.RGBA, gl.UNSIGNED_BYTE, lut); } gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, this.toneTexture); try { gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, this.videoTexture); if (this._texW !== w || this._texH !== h) { this._texW = w; this._texH = h; const internalFormat = this._isGL2 ? gl.RGBA8 : gl.RGBA; gl.texImage2D(gl.TEXTURE_2D, 0, internalFormat, w, h, 0, gl.RGBA, gl.UNSIGNED_BYTE, null); } gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, gl.RGBA, gl.UNSIGNED_BYTE, video); gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4); st.webglFailCount = 0; if (!this._outputReady) { this._outputReady = true; this.canvas.style.opacity = '1'; if (!this.restoreVideoStyle) this.restoreVideoStyle = patchInlineStyleImportant(video, { opacity: '0.001' }); } } catch (err) { st.webglFailCount = (st.webglFailCount || 0) + 1; if (CONFIG.DEBUG) log.warn('WebGL render failure:', err); const msg = String(err?.message || err || ''); const looksTaint = /SecurityError|cross.origin|cross-origin|taint|insecure|Tainted|origin/i.test(msg); if (st.webglFailCount >= RUNTIME_GUARD.webgl.failThreshold) { st.webglFailCount = 0; if (looksTaint) { st.webglTainted = true; log.warn('WebGL tainted/CORS-like failure → fallback to SVG'); } else { if (VSC_DEFENSE.webglCooldown) st.webglDisabledUntil = now + RUNTIME_GUARD.webgl.failCooldownMs; log.warn('WebGL transient failure → cooldown then retry'); } try { window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} } } }
        ...
        scheduleNextFrame(loopFn) { if (document.hidden) { this._loopRunning = false; return; } if (this.video?.paused) { this._loopRunning = false; return; } this.renderDriver.scheduleVideoFrame(this.video, loopFn, 'raf', 90); }
        ...
        shutdown() {
          ...
          try { this.restoreVideoStyle?.(); } catch (_) {} this.restoreVideoStyle = null; try { if (this.canvas && this.canvas.parentNode) { this.canvas.remove(); } } catch (_) {} if (this._parentStylePatched && this._patchedParent) { try { this._patchedParent.style.position = this._parentPrevPosition; } catch (_) {} this._parentStylePatched = false; this._parentPrevPosition = ''; this._patchedParent = null; } this.disposeGLResources();
        }
      }
      return { apply: (el, vVals) => { let pipe = pipelines.get(el); if (!pipe) { pipe = new WebGLPipeline(); pipelines.set(el, pipe); } if (!pipe.active || pipe.video !== el || !pipe.gl) { if (!pipe.attachToVideo(el)) { pipelines.delete(el); return false; } } pipe.updateParams(vVals); if (!pipe._loopRunning && !el.paused && !document.hidden) { pipe.startRenderLoop(); } return true; }, clear: (el) => { const pipe = pipelines.get(el); if (pipe) { pipe.shutdown(); pipelines.delete(el); } } };
    }
const __styleCache = new Map();
    ...
    function showOSD(text, durationMs = 1200) {
      ...
      let el = document.getElementById('vsc-osd');
      if (!el) {
        el = document.createElement('div'); el.id = 'vsc-osd';
        el.style.cssText = `position: fixed; top: 60px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.82); color: #fff; padding: 8px 20px; border-radius: 8px; font: 600 14px/1.4 sans-serif; z-index: 2147483647; pointer-events: none; opacity: 0; transition: opacity 0.2s; backdrop-filter: blur(6px);`;
        (document.body || document.documentElement).appendChild(el);
      }
      el.textContent = text; el.style.opacity = '1';
      clearTimeout(el._timer); el._timer = setTimeout(() => { el.style.opacity = '0'; }, durationMs);
    }
    ...
    function createUI(sm, registry, ApplyReq, Utils) {
      ...
      function renderButtonRow({ label, items, key, offValue = null, toggleActiveToOff = false }) { const row = h('div', { class: 'prow' }, h('div', { style: 'font-size:11px;width:35px;line-height:34px;font-weight:bold' }, label)); const addBtn = (text, value) => { const b = h('button', { class: 'pbtn', style: 'flex:1' }, text); b.onclick = () => { const cur = sm.get(key); if (toggleActiveToOff && offValue !== undefined && cur === value && value !== offValue) { setAndHint(key, offValue); } else { setAndHint(key, value); } }; bindClassToggle(b, key, v => v === value); row.append(b); }; for (const it of items) addBtn(it.text, it.value); if (offValue !== undefined && offValue !== null && !items.some(it => it.value === offValue)) addBtn('OFF', offValue); return row; }
      function renderShadowBandMaskRow({ label = '블랙', key = P.V_SHADOW_MASK }) { const row = h('div', { class: 'prow' }, h('div', { style: 'font-size:11px;width:35px;line-height:34px;font-weight:bold' }, label) ); const items = [ { text: '외암', bit: SHADOW_BAND.OUTER, title: '옅은 암부 진하게 (중간톤 대비 향상)' }, { text: '중암', bit: SHADOW_BAND.MID, title: '가운데 암부 진하게 (무게감 증가)' }, { text: '심암', bit: SHADOW_BAND.DEEP, title: '가장 진한 블랙 (들뜬 블랙 제거)' } ]; for (const it of items) { const b = h('button', { class: 'pbtn', style: 'flex:1', title: it.title }, it.text); b.onclick = () => { sm.set(key, ShadowMask.toggle(sm.get(key), it.bit)); ApplyReq.hard(); }; bindClassToggle(b, key, v => ShadowMask.has(v, it.bit)); row.append(b); } const off = h('button', { class: 'pbtn', style: 'flex:0.9' }, 'OFF'); off.onclick = () => { sm.set(key, 0); ApplyReq.hard(); }; bindClassToggle(off, key, v => (Number(v) | 0) === 0); row.append(off); return row; }
      ...
      const build = () => {
        ...
        applyShadowStyle(shadow, style, h); const dragHandle = h('div', { class: 'header' }, 'VSC 렌더링 제어');
        const rmBtn = h('button', { id: 'rm-btn', class: 'btn', onclick: () => setAndHint(P.APP_RENDER_MODE, sm.get(P.APP_RENDER_MODE) === 'webgl' ? 'svg' : 'webgl') }); bindStyle(rmBtn, P.APP_RENDER_MODE, (el, v) => { el.textContent = `🎨 ${v === 'webgl' ? 'WebGL' : 'SVG'}`; el.style.color = v === 'webgl' ? '#ffaa00' : '#88ccff'; el.style.borderColor = v === 'webgl' ? '#ffaa00' : '#88ccff'; });
        const boostBtn = h('button', { id: 'boost-btn', class: 'btn', onclick: () => setAndHint(P.A_EN, !sm.get(P.A_EN)) }, '🔊 Brickwall'); bindClassToggle(boostBtn, P.A_EN, v => !!v);
        const pipBtn = h('button', { class: 'btn', onclick: async () => { const v = window.__VSC_APP__?.getActiveVideo(); if(v) await togglePiPFor(v); } }, '📺 PIP');
        const zoomBtn = h('button', { id: 'zoom-btn', class: 'btn', onclick: () => { const nextEn = !sm.get(P.APP_ZOOM_EN); setAndHint(P.APP_ZOOM_EN, nextEn); const zm = window.__VSC_INTERNAL__.ZoomManager; const v = window.__VSC_APP__?.getActiveVideo(); if (zm && v) { if (zm.isZoomed(v)) { zm.resetZoom(v); } else { const rect = v.getBoundingClientRect(); zm.zoomTo(v, 1.5, rect.left + rect.width / 2, rect.top + rect.height / 2); } } } }, '🔍 줌 제어'); bindClassToggle(zoomBtn, P.APP_ZOOM_EN, v => !!v);
        const autoSceneBtn = h('button', { class: 'btn', onclick: () => setAndHint(P.APP_AUTO_SCENE, !sm.get(P.APP_AUTO_SCENE)) }, '✨ Auto'); bindClassToggle(autoSceneBtn, P.APP_AUTO_SCENE, v => !!v);
        const pwrBtn = h('button', { id: 'pwr-btn', class: 'btn', onclick: () => setAndHint(P.APP_ACT, !sm.get(P.APP_ACT)) }, '⚡ Power'); bindStyle(pwrBtn, P.APP_ACT, (el, v) => { el.style.color = v ? '#2ecc71' : '#e74c3c'; });
        const advToggleBtn = h('button', { class: 'btn', style: 'width: 100%; margin-bottom: 6px; background: #2c3e50; border-color: #34495e;' }, '▼ 고급 설정 열기'); advToggleBtn.onclick = () => setAndHint(P.APP_ADV, !sm.get(P.APP_ADV)); bindStyle(advToggleBtn, P.APP_ADV, (el, v) => { el.textContent = v ? '▲ 고급 설정 닫기' : '▼ 고급 설정 열기'; el.style.background = v ? '#34495e' : '#2c3e50'; });
        const advContainer = h('div', { style: 'display: none; flex-direction: column; gap: 0px;' }, [ renderShadowBandMaskRow({ label: '블랙', key: P.V_SHADOW_MASK }), renderButtonRow({ label: '복구', key: P.V_BRIGHT_STEP, offValue: 0, toggleActiveToOff: true, items: [{ text: '1단', value: 1 }, { text: '2단', value: 2 }, { text: '3단', value: 3 }] }), renderButtonRow({ label: '밝기', key: P.V_PRE_B, offValue: 'brOFF', toggleActiveToOff: true, items: Object.keys(PRESETS.grade).filter(k=>k!=='brOFF').map(k => ({ text: k, value: k })) }) ]); bindStyle(advContainer, P.APP_ADV, (el, v) => { el.style.display = v ? 'flex' : 'none'; });
        const bodyMain = h('div', { id: 'p-main' }, [ h('div', { class: 'prow' }, [ rmBtn, autoSceneBtn ]), h('div', { class: 'prow' }, [ pipBtn, zoomBtn, boostBtn ]), h('div', { class: 'prow' }, [ h('button', { class: 'btn', onclick: () => sm.set(P.APP_UI, false) }, '✕ 닫기'), pwrBtn, h('button', { class: 'btn', onclick: () => { sm.batch('video', DEFAULTS.video); sm.batch('audio', DEFAULTS.audio); sm.batch('playback', DEFAULTS.playback); sm.set(P.APP_AUTO_SCENE, false); ApplyReq.hard(); } }, '↺ 리셋') ]), renderButtonRow({ label: '샤프', key: P.V_PRE_S, offValue: 'off', toggleActiveToOff: true, items: Object.keys(PRESETS.detail).filter(k=>k!=='off').map(k => ({ text: k, value: k })) }), advToggleBtn, advContainer, h('hr'), h('div', { class: 'prow', style: 'justify-content:center;gap:4px;flex-wrap:wrap;' }, [0.5, 1.0, 1.5, 2.0, 3.0, 5.0].map(s => { const b = h('button', { class: 'pbtn', style: 'flex:1;min-height:36px;' }, s + 'x'); b.onclick = () => { setAndHint(P.PB_RATE, s); setAndHint(P.PB_EN, true); }; bindRateButtonActive(b, s, sm, sub, P); return b; })) ]);
        const mainPanel = h('div', { class: 'main' }, [ dragHandle, bodyMain ]); shadow.append(mainPanel); let stopDrag = null;
        dragHandle.addEventListener('mousedown', (e) => { e.preventDefault(); stopDrag?.(); let startX = e.clientX, startY = e.clientY; const rect = mainPanel.getBoundingClientRect(); mainPanel.style.transform = 'none'; mainPanel.style.top = `${rect.top}px`; mainPanel.style.right = 'auto'; mainPanel.style.left = `${rect.left}px`; stopDrag = bindWindowDrag((ev) => { const dx = ev.clientX - startX, dy = ev.clientY - startY, panelRect = mainPanel.getBoundingClientRect(); let nextLeft = Math.max(0, Math.min(window.innerWidth - panelRect.width, rect.left + dx)), nextTop = Math.max(0, Math.min(window.innerHeight - panelRect.height, rect.top + dy)); mainPanel.style.left = `${nextLeft}px`; mainPanel.style.top = `${nextTop}px`; }, () => { stopDrag = null; }); });
    ...
    // ✨ 패치 1-6: Adapter 참조 방어코드
    const onEvictVideo = (v) => { try { window.__VSC_INTERNAL__?.Adapter?.clear(v); } catch (_) {} restoreRateOne(v); };
    ...
    // ✨ 패치 4-2: makeLightSig 문자열 최적화
    function makeLightSig({ activeTarget, rMode, pbActive, desiredRate, videoFxOn, storeRev }) { return storeRev + '|' + getElemId(activeTarget) + '|' + rMode + '|' + (pbActive ? 1 : 0) + '|' + (desiredRate ?? 1) + '|' + (videoFxOn ? 1 : 0); }
    ...
    function applyPlaybackRate(el, desiredRate) {
      const st = getVState(el); const rSt = getRateState(el); if (rSt.orig == null) rSt.orig = el.playbackRate; const lastDesired = st.desiredRate;
      if (!Object.is(lastDesired, desiredRate) || Math.abs(el.playbackRate - desiredRate) > 0.01) { st.desiredRate = desiredRate; markInternalRateChange(el, 160); try { el.playbackRate = desiredRate; } catch (_) {} }
    ...
    function createAppController({ Store, Registry, Scheduler, ApplyReq, Adapter, Audio, UI, Utils, P, Targeting }) {
      ...
      let qualityScale = 1.0; let lastQCheck = 0;
      function updateQualityScale(v) { if (!v || typeof v.getVideoPlaybackQuality !== 'function') return qualityScale; const now = performance.now(); if (now - lastQCheck < 2000) return qualityScale; lastQCheck = now; try { const q = v.getVideoPlaybackQuality(); const dropped = Number(q.droppedVideoFrames || 0); const total = Number(q.totalVideoFrames || 0); const ratio = total > 0 ? (dropped / total) : 0; qualityScale = ratio > 0.08 ? 0.65 : ratio > 0.04 ? 0.80 : 1.0; } catch (_) {} return qualityScale; }
      Scheduler.registerApply((force) => {
        try {
          ...
          // ✨ 패치 4-4: Proxy 핫패스 회피를 위한 순수 객체 구성
          const vfUser = { presetS: Store.get(P.V_PRE_S), presetB: Store.get(P.V_PRE_B), presetMix: Store.get(P.V_PRE_MIX), shadowBandMask: Store.get(P.V_SHADOW_MASK), brightStepLevel: Store.get(P.V_BRIGHT_STEP) };
          let vValsEffective = videoParamsMemo.get(vfUser, rMode, __activeTarget);
          const qs = updateQualityScale(__activeTarget);
          if (qs !== 1.0) { vValsEffective = { ...vValsEffective }; vValsEffective.sharp = (vValsEffective.sharp || 0) * qs; vValsEffective.sharp2 = (vValsEffective.sharp2 || 0) * qs; vValsEffective.clarity = (vValsEffective.clarity || 0) * qs; }
          debugLogEffectiveVVals(vValsEffective, vfUser, __activeTarget, rMode);
          ...
          reconcileVideoEffects({ applySet: __applySet, dirtyVideos: vidsDirty, vVals: vValsEffective, videoFxOn, desiredRate, pbActive, Adapter, rMode, ApplyReq });
    ...
    window.addEventListener('message', (e) => { if (e.data && e.data.__vsc_sync && e.data.token === VSC_SYNC_TOKEN) { const { p, val } = e.data; if (p === P.APP_UI) return; if (Object.values(P).includes(p)) { if (Store.get(p) !== val) Store.set(p, val); } } });
    ...
    bindNormalizer([P.APP_RENDER_MODE, P.APP_APPLY_ALL, P.APP_ZOOM_EN, P.APP_AUTO_SCENE, P.APP_ADV], APP_SCHEMA); bindNormalizer([P.V_PRE_S, P.V_PRE_B, P.V_PRE_MIX, P.V_SHADOW_MASK, P.V_BRIGHT_STEP], VIDEO_SCHEMA); bindNormalizer([P.A_EN, P.A_BST, P.PB_EN, P.PB_RATE], AUDIO_PLAYBACK_SCHEMA);

    // ✨ 패치 7-2: OSD 알림 바인딩
    Store.sub(P.V_PRE_S, (v) => showOSD('샤프닝: ' + v));
    ...
    waitForVisibility().then(() => {
      ...
      // ✅ [지점 0] 최종 정식 본문 호출부 수정
      const Filters = createFiltersVideoOnly(Utils, { VSC_ID: CONFIG.VSC_ID, IS_LOW_END: CONFIG.IS_LOW_END, IS_MOBILE: CONFIG.IS_MOBILE });
      ...
      onWin('pointerdown', (e) => { const now = performance.now(); updateLastUserPt(e.clientX, e.clientY, now); signalUserInteractionForRetarget(); }, { passive: true }); onWin('wheel', (e) => { const x = Number.isFinite(e.clientX) ? e.clientX : innerWidth * 0.5; const y = Number.isFinite(e.clientY) ? e.clientY : innerHeight * 0.5; updateLastUserPt(x, y, performance.now()); signalUserInteractionForRetarget(); }, { passive: true }); onWin('keydown', () => { updateLastUserPt(innerWidth * 0.5, innerHeight * 0.5, performance.now()); signalUserInteractionForRetarget(); }); onWin('resize', () => { const now = performance.now(); if (!window.__lastUserPt || (now - window.__lastUserPt.t) > 1200) updateLastUserPt(innerWidth * 0.5, innerHeight * 0.5, now); signalUserInteractionForRetarget(); }, { passive: true });
      ...
      (function addPageLifecycleHooks() { onWin('freeze', () => { try { window.__VSC_INTERNAL__?.App?.getActiveVideo() && window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} }, { capture: true }); onWin('pageshow', () => { try { window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} }, { capture: true }); onDoc('visibilitychange', () => { try { if (document.visibilityState === 'visible') window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} }, { passive: true }); onWin('resume', () => { try { window.__VSC_INTERNAL__?.ApplyReq?.hard(); } catch (_) {} }, { capture: true }); })();
