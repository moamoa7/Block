// ==UserScript==
// @name         Video_Control (v185.4 - Refined & Patched)
// @namespace    https://github.com/
// @version      185.4
// @description  Critical bug fixes (PiP Lock, AudioWorklet Leak, Ratechange Race) & Perf Boost (Modern API, Config Refactor).
// @match        *://*/*
...
function VSC_MAIN() {
  ...
  function destroyRuntime(ns = __vscNs) {
    ...
    // [PATCH 1.5] 불필요한 typeof __shadowRootCallbacks 체크 제거
    try { if (ns._shadowRootCb && __shadowRootCallbacks) { __shadowRootCallbacks.delete(ns._shadowRootCb); } } catch (_) {}
  ...
  // =========================================================================
  // [Refactor 3.7] CONFIG SECTION: 상수 및 스키마 상단 분리 배치
  // =========================================================================
  ...
  // [PATCH 2.1] AbortSignal.any() 2024+ 표준 API 사용으로 메모리 누수 방어 및 간소화
  ...
  function createEventBus() {
    const _listeners = new Map(), _onceListeners = new Map();
    let _destroyed = false, _hasWildcard = false;
    ...
    return Object.freeze({
      ...
      off(event, handler) {
        if (handler) {
          ...
          if (event === '*' && !_listeners.get('*')?.size && !_onceListeners.get('*')?.size) { _hasWildcard = false; }
        } else {
          _listeners.delete(event);
          _onceListeners.delete(event);
          if (event === '*') _hasWildcard = false;
        }
      },
      destroy() { _destroyed = true; _listeners.clear(); _onceListeners.clear(); _hasWildcard = false; },
  ...
  function defineFeature(spec) {
    ...
    const module = {
      ...
      destroy() {
        ...
        if (typeof spec.onDestroy === 'function') { try { spec.onDestroy.call(_helpers); } catch (e) { console.warn(`[VSC ${_name}] onDestroy error:`, e); } }
  ...
  function createFeatureRegistry(bus) {
    ...
    return Object.freeze({
      ...
      initAll(deps) {
        if (_initialized) return;
        _initialized = true;
        for (const [name, mod] of _getSorted()) { try { mod.init(deps); } catch (e) { console.warn(`[VSC] "${name}" init failed:`, e); } }
        bus.emit('features:initialized', { count: _modules.size, names: _getSorted().map(([n]) => n) });
      },
      updateAll(ctx) {
        for (const [name, mod] of _getSorted()) { try { mod.update(ctx); } catch (e) { if (ctx.force) console.warn(`[VSC] "${name}" update failed:`, e); } }
      },
      destroyAll() {
        const entries = [..._getSorted()].reverse();
        for (const [name, mod] of entries) { try { mod.destroy(); } catch (e) { console.warn(`[VSC] "${name}" destroy:`, e); } }
  ...
  const getVState = (v) => {
    ...
    if (!st) {
      st = {
        visible: false, bound: false, applied: false, desiredRate: undefined, audioFailUntil: 0, _lastSrc: '', _ac: null, visibilityRatio: 0,
        rect: null, rectT: 0, _rectRev: 0, origFilter: null, origFilterPrio: '', origWebkitFilter: null, origWebkitFilterPrio: '',
        lastFilterUrl: null, lastTransform: undefined, lastScale: undefined, lastRot: undefined, rateState: null
      };
  ...
  function createLocalStore(defaults, scheduler, bus) {
    // [PATCH 2.5] structuredClone 네이티브 지원 100% 활용하여 fallback 제거
    ...
    function mergeKnown(dst, src, defaultsObj) {
      ...
      for (const key of Object.keys(defaultsObj)) {
        if (Object.prototype.hasOwnProperty.call(src, key)) {
          ...
          else if (typeof defaultsObj[key] === 'number') dst[key] = Number(src[key]) || 0;
  ...
  function createRegistry(scheduler, bus) {
    ...
    const WorkQ = (() => {
      ...
      const drain = async (dl) => {
        ...
        if (active.length <= 64) {
          for (let i = 0; i < active.length; i++) { activeSet.delete(active[i]); safe(() => scanNode(active[i])); }
          active.length = 0; activeSet.clear(); return;
        }
        // [PATCH 2.2] 글로벌 스케줄러를 직접 참조하여 ReferenceError 원천 차단
        const canYield = globalThis.scheduler?.yield != null;
        const start = performance.now(); const iip = navigator.scheduling?.isInputPending?.bind(navigator.scheduling); const maxMs = dl?.timeRemaining ? Math.min(dl.timeRemaining(), 10) : 4; let processed = 0;
...
// =================== PART 2 START ===================
  class PiPStateManager {
    ...
    reset() {
      // [PATCH 1.3] 상태 해제를 최상단으로 이동하여 예외 발생 시 PiP 영구 잠금 방지
      this._restoring = false;
      try { this._uiCleanup?.(); } catch (_) {}
      this._uiCleanup = null;
      if (this._ac) {
        try { this._ac.abort(); } catch (_) {} // abort 자체 예외 방어
        this._ac = null;
      }
  ...
  async function _restoreVideoCommon(video, { adoptNode: shouldAdopt = false } = {}) {
    ...
    try {
      ...
      log.warn('[VSC PiP] restore failed:', e);
    } finally {
      PiPState.reset();
    }
  }
...
`;
  ...
  function createAudioFeature(sm) {
    ...
    async function ensureFinalizerWorklet(audioCtx) {
      // [PATCH 1.4] 컨텍스트 일치 여부 엄격 검증 및 Stale Worklet 누수 차단
      ...
      if (workletInitPromise) {
        ...
        if (result && result.context === audioCtx) return result;
      ...
      workletInitPromise = (async () => {
        ...
        try {
          ...
          if (audioCtx.state === 'closed') {
            finalizerNode.disconnect(); finalizerNode = null; return null;
          }
    ...
    function ensureVisibilityResumeHook() {
      ...
      on(document, 'visibilitychange', () => {
        ...
        if (document.visibilityState === 'visible') {
          ...
          if (!PiPState.isActive && !dynAct && ctx.state === 'running') { ctx.suspend().catch(() => {}); }
        }
...
// =================== PART 3 START ===================
  ...
  function createUI(sm, registry, ApplyReq, Utils, P, Bus) {
    ...
    // [PATCH 2.4] CSSStyleSheet 조작 최적화 (배열 복사 대신 push 활용)
    function attachShadowStyles(shadowRoot, cssText) {
      try {
        if ('adoptedStyleSheets' in shadowRoot && typeof CSSStyleSheet !== 'undefined') {
          ...
          if (shadowRoot.adoptedStyleSheets.push) {
  ...
  const bindVideoOnce = (v, ApplyReq) => {
    ...
    const videoEvents = [
      ...
      ['play', () => { ApplyReq.hard(); }],
      // [PATCH 1.2] 마이크로태스크 갭을 악용한 Ratechange Race Condition 완전 해결
      ['ratechange', () => {
        ...
        const capturedRate = v.playbackRate; // 시점 값 캡처
        ...
        queueMicrotask(() => {
          ...
          const rSt2 = getRateState(v);
          if (performance.now() < (rSt2.suppressSyncUntil || 0)) return;
          // 캡처 이후 다른 변경이 개입했는지 확인
  ...
  function createVideoParamsMemo() {
    ...
    return {
      get(vfUser, video, budget) {
        ...
        // [PATCH USER DATA] User request: '18로 원복' -> _microBase: 0.18 fallback apply
  ...
  function createTimerFeature() {
    ...
    function tick() {
      ...
      if (!act || !timeEn || !isFs) {
        ...
        _lastSecond = -1; _lastLayoutKey = ''; scheduleNext(); return;
      }
    ...
    return defineFeature({
      ...
      onInit() {
        ...
        this.subscribe('fullscreen:changed', ({ active }) => {
          if (!active && _timerEl) { _timerEl.style.display = 'none'; _lastSecond = -1; _lastLayoutKey = ''; }
