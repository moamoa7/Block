// ==UserScript==
// @name         Video_Control (v170.8.0 - Zero-Idle WebGL & Adaptive SVG / Alpha Patched)
...
(function () {
  ...
  function VSC_MAIN() {
    ...
    const VSC_VERSION = '170.8.0'; const VSC_SYNC_TOKEN = `VSC_SYNC_${VSC_VERSION}_${CONFIG.VSC_ID}`;
    const VSC_CLAMP = (v, min, max) => (v < min ? min : (v > max ? max : v));

    function tempToRgbGain(temp) {
      const clamp = VSC_CLAMP; const t = clamp((Number(temp) || 0) / 50, -1, 1); let r = 1 + 0.10 * t, b = 1 - 0.10 * t, g = 1 - 0.04 * Math.abs(t); const m = Math.max(r, g, b); return { rs: r / m, gs: g / m, bs: b / m };
    }
    ...
    function composeBaseVideoParams(out, vUser) { const mix = VSC_CLAMP(vUser.presetMix ?? 1.0, 0, 1); const pD = PRESETS.detail[vUser.presetS] || PRESETS.detail.off; const pB = PRESETS.grade[vUser.presetB] || PRESETS.grade.brOFF; const preGammaF = lerp(1.0, pB.gammaF ?? 1.0, mix); const preBright = (pB.brightAdd || 0) * mix; let preSharp = (pD.sharpAdd || 0) * mix; let preSharp2 = (pD.sharp2Add || 0) * mix; let preClarity = (pD.clarityAdd || 0) * mix; out.gain = 1.0; out.gamma = preGammaF; out.contrast = 1.0; out.satF = 1.0; out.bright = preBright; out.temp = 0; out.mid = 0; out.toe = 0; out.shoulder = 0; out.sharp = Math.min(preSharp, 150); out.sharp2 = Math.min(preSharp2, 150); out.clarity = Math.min(preClarity, 150); return out; }
    ...
    // ⭐ v170.7.0 패치: Alpha 전파 수정 + 모바일 샤프닝 적응형 분기
    function createFiltersVideoOnly(Utils, config) {
      ...
      function buildSvg(root) {
        ...
        // ✨ 핵심 수정: edgeGate에 forceOpaqueAlpha=true
        ...
        // ✨ 핵심 수정: invGate에도 forceOpaqueAlpha=true
        ...
        return {
          ...
          full: {
            ...
            casLike: {
              rngMax, rngMin, rngBlur,
              edgeGateFuncs: Array.from(edgeGate.children).filter(n => n.tagName !== 'feFuncA'),
              hlKeepFuncs: Array.from(hlKeep.children),
              sharpDesat, sharpBiased
            }
          },
      ...
      function prepare(video, s) {
        ...
        if (ctx.st.lastKey !== key) {
          ...
          const ck = [ sharpTotal.toFixed(2), (s.bright || 0).toFixed(2), (s.gamma || 1).toFixed(3), (s.contrast || 1).toFixed(3), vw, vh, morphRadius ].join('|');
          ...
          queueMicrotask(() => {
            ...
            if (filterGrade === 'M' && detailOn) {
              ...
              if (st.midConvKey !== mk) {
                st.midConvKey = mk;
                const midSharpMul = config.IS_MOBILE ? 0.60 : 0.30;
                const rawS = ((s.sharp || 0) + (s.sharp2 || 0) * 0.40 + (s.clarity || 0) * 0.18) / 50.0;
                const totalS = Math.min(0.65, rawS * midSharpMul);
            ...
            if (filterGrade === 'F' && detailOn) {
              ...
              if (st.detailKey !== dk) {
                ...
                const fullSharpMul = config.IS_MOBILE ? 0.80 : 0.55;

                const v1 = (s.sharp || 0) / 50 * fullSharpMul;
                ...
                const v2 = (s.sharp2 || 0) / 50 * fullSharpMul;
                ...
                const clVal = (s.clarity || 0) / 50 * fullSharpMul;
              ...
              if (st.casKey !== ck) {
                ...
                // ✨ CAS Full-Bypass Alpha Patch 적용 완료
                if (morphRadius === 0) {
                  const morphKey = '0';
                  ...
                  if (st.__sharpBiasK2 !== '1' || st.__sharpBiasK3 !== '0') {
                    st.__sharpBiasK2 = '1';
                    st.__sharpBiasK3 = '0';
                    fn.casLike.sharpBiased.setAttribute('k2', '1');
...
void main() {
  ...
  outColor = vec4(clamp(color, 0.0, 1.0), 1.0);
      ...
      class WebGLPipeline {
        ...
        attachToVideo(video) { if (!this.active && !this.init()) return false; this.video = video; this.originalParent = video.parentNode; this.restoreVideoStyle = null; this._outputReady = false; this.canvas.style.opacity = '0'; if (this.originalParent) { const cs = window.getComputedStyle(this.originalParent); if (cs.position === 'static') { this._parentPrevPosition = this.originalParent.style.position || ''; this.originalParent.style.position = 'relative'; this._parentStylePatched = true; this._patchedParent = this.originalParent; } if (video.nextSibling) this.originalParent.insertBefore(this.canvas, video.nextSibling); else this.originalParent.appendChild(this.canvas); } if (this._styleObs) this._styleObs.disconnect(); this._styleObs = new MutationObserver(() => { this._styleDirty = true; }); try { this._styleObs.observe(video, { attributes: true, attributeFilter: ['style', 'class'] }); } catch (_) {} this._styleDirty = true; this.active = true; this.startRenderLoop(); return true; }
        ...
        shutdown() { this.active = false; this._loopToken++; this._loopRunning = false; this.renderDriver.clear(); if (this._styleObs) { this._styleObs.disconnect(); this._styleObs = null; } try { this.restoreVideoStyle?.(); } catch (_) {} this.restoreVideoStyle = null; try { if (this.canvas && this.canvas.parentNode) { this.canvas.remove(); } } catch (_) {} if (this._parentStylePatched && this._patchedParent) { try { this._patchedParent.style.position = this._parentPrevPosition; } catch (_) {} this._parentStylePatched = false; this._parentPrevPosition = ''; this._patchedParent = null; } this.disposeGLResources(); }
      }
    ...
    function createAppController({ Store, Registry, Scheduler, ApplyReq, Adapter, Audio, UI, Utils, P, Targeting }) {
      ...
      Scheduler.registerApply((force) => {
        try {
          ...
          applySet = new Set(); if (applyToAllVisibleVideos) { for (const v of visible.videos) applySet.add(v); } else if (__activeTarget) { applySet.add(__activeTarget); }
    ...
    waitForVisibility().then(() => {
      ...
      onWin('keydown', async (e) => { if (isEditableTarget(e.target)) return; if (e.altKey && e.shiftKey && e.code === 'KeyV') { e.preventDefault(); e.stopPropagation(); try { Store.set(P.APP_UI, !Store.get(P.APP_UI)); ApplyReq.hard(); } catch (_) {} return; } if (!(e.altKey && e.shiftKey && e.code === 'KeyP')) return; const v = __VSC_APP__?.getActiveVideo(); if (!v) return; await togglePiPFor(v); }, { capture: true });
