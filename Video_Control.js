// ==UserScript==
// @name         Video_Control (v28.9.10)
// @namespace    https://github.com/
// @version      28.9.10
// @description  v28.9.10: (최종) 밝기 180에서 Clarity 5로 정확히 하향 매핑 + 하이라이트 보호 완벽 통합
// @match        *://*/*
...
(function () {
  ...
  function createRegistry(scheduler) {
    ...
    // 오타 수정 확실히 반영된 IntersectionObserver
  ...
  function createFilters() {
    ...
    /* ── [v28.9.10] 톤 테이블: Clarity S커브 + 하이라이트 보호 적용 ── */
    function getToneTable(steps, gain, contrast, gamma, toe, mid, shoulder, highRoll, clarity) {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        /* ── Method 3: Clarity 픽셀 단위 하이라이트 보호 ── */
        if (cl > 0.001) {
          const cCenter = 0.50;
          const cSigma = 0.25;
          ...
          // 밝은 영역(x0 > 0.70)에서 더 밝아지는 방향의 연산 억제
          const hlProtect = (deviation > 0 && x0 > 0.70)
  ...
  function createAutoScene(store, scheduler) {
    ...
    /* ── [v28.9.10] 밝기 180에서 정확히 5 적용 ── */
    function getAutoClarity(frameBrightness) {
      if (frameBrightness <= 130) return 0;
      if (frameBrightness <= 190) return Math.round((frameBrightness - 130) / 60 * 6);
      if (frameBrightness <= 230) return Math.round(6 + (frameBrightness - 190) / 40 * 8);
      return Math.round(14 + Math.min(frameBrightness - 230, 25) / 25 * 4);
    }
    ...
    /* ── Method 2: Clarity-HighRoll 자동 페어링 ── */
