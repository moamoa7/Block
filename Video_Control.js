// ==UserScript==
// @name         Video_Control (v171.1.0 - Zero-Idle WebGL & Adaptive SVG)
...
(function () {
  ...
  function VSC_MAIN() {
    ...
    function on(target, type, fn, opts = {}) {
      try { target.addEventListener(type, fn, { ...opts, signal: __globalSig }); } catch (_) {}
    }
    ...
    function createAutoSceneManager(Store, P, Scheduler) {
      ...
      function computeStatsAndMotion(AUTO, img, sw, sh) {
        const data = img.data;
        const samplesW = (sw + 1) >> 1;
        const samplesH = (sh + 1) >> 1;
        const n = samplesW * samplesH;
        ...
        let sum = 0, sum2 = 0, sumEdge = 0, diffSum = 0, sumChroma = 0;
        let pi = 0;
        for (let y = 0; y < sh; y += 2) {
          ...
          for (let x = 0; x < sw; x += 2) {
            ...
            cur[pi] = l;
            sum += l; sum2 += l * l;
            if (x + 2 < sw && idx + 10 < data.length) { const r2 = data[idx + 8], g2 = data[idx + 9], b2 = data[idx + 10]; const l2 = (r2 * 0.2126 + g2 * 0.7152 + b2 * 0.0722) | 0; sumEdge += Math.abs(l2 - l); }
            if (y + 2 < sh) { const idxD = idx + (sw * 2 * 4); if (idxD + 2 < data.length) { const r3 = data[idxD], g3 = data[idxD + 1], b3 = data[idxD + 2]; const l3 = (r3 * 0.2126 + g3 * 0.7152 + b3 * 0.0722) | 0; sumEdge += Math.abs(l3 - l); } }
            diffSum += Math.abs(l - prev[pi]);
            pi++; idx += 8;
          }
        }
        AUTO._lumaFlip ^= 1;
        const samples = Math.max(1, n);
        const mean = sum / samples; const var_ = (sum2 / samples) - mean * mean; const std = Math.sqrt(Math.max(0, var_));
        const edge = sumEdge / samples; const motion = diffSum / samples; const bright = mean / 255; const contrast = std / 64; const chroma = (sumChroma / samples) / 255;
    ...
    function createFiltersVideoOnly(Utils, config) {
      ...
      function buildSvg(root) {
        ...
        /* ── v171.1.0 patch: ShadowRoot-aware SVG insertion ── */
        const tryAppend = () => {
          const target = (root instanceof ShadowRoot)
            ? root
...
void main() {
  vec3 color = TEX(uVideoTex, vTexCoord).rgb; color *= uRGBGain; color += (uParams2.x / 1000.0); color = (color - 0.5) * uParams.y + 0.5;
...
void main() {
  ...
  color *= uRGBGain; color += (uParams2.x / 1000.0); color = (color - 0.5) * uParams.y + 0.5;
  float y = dot(color, LUMA); float y2 = tone1(clamp(y, 0.0, 1.0)); float ratio = y2 / max(1e-4, y); color *= ratio;
  float luma = dot(color, LUMA); float hiLuma = clamp((luma - 0.72) / 0.28, 0.0, 1.0); float satReduce = hiLuma * hiLuma * (3.0 - 2.0 * hiLuma); float currentSat = uParams.z * (1.0 - 0.05 * satReduce); color = luma + (color - luma) * currentSat;
  if (uParams.w != 1.0) color = pow(max(color, vec3(0.0)), vec3(1.0 / uParams.w)); color = softClip(color, 0.18);
  outColor = vec4(clamp(color, 0.0, 1.0), 1.0);
      ...
      class WebGLPipeline {
        ...
        scheduleNextFrame(loopFn) { if (document.hidden) { this._loopRunning = false; return; } if (this.video?.paused) { this._loopRunning = false; if (this._paramsDirty) { requestAnimationFrame(() => { this.render(); }); } return; } this.renderDriver.scheduleVideoFrame(this.video, loopFn, 'raf', 90); }
      ...
      const build = () => {
        ...
        const videoSection = h('div', { class: 'section' },
          ...
          h('div', { class: 'row', style: 'margin-top:6px' },
            (() => { const rmBtn = h('button', { class: 'btn' }); bindStyle(rmBtn, P.APP_RENDER_MODE, (el, v) => { el.innerHTML = ''; el.append(h('span', { class: 'icon' }, v === 'webgl' ? '🎮' : '🎨'), document.createTextNode(v === 'webgl' ? 'WebGL' : 'SVG')); el.classList.toggle('active', v === 'webgl'); }); rmBtn.onclick = () => setAndHint(P.APP_RENDER_MODE, sm.get(P.APP_RENDER_MODE) === 'webgl' ? 'svg' : 'webgl'); return rmBtn; })(),
            (() => { const autoBtn = h('button', { class: 'btn' }, h('span', { class: 'icon' }, '✨'), 'Auto'); autoBtn.onclick = () => setAndHint(P.APP_AUTO_SCENE, !sm.get(P.APP_AUTO_SCENE)); bindClassToggle(autoBtn, P.APP_AUTO_SCENE, v => !!v); return autoBtn; })()
          )
        );

        const audioSection = h('div', { class: 'section' },
          h('div', { class: 'section-label' }, '오디오'),
          h('div', { class: 'row' },
            (() => { const boostBtn = h('button', { class: 'btn' }, h('span', { class: 'icon' }, '🔊'), 'Brickwall'); boostBtn.onclick = () => setAndHint(P.A_EN, !sm.get(P.A_EN)); bindClassToggle(boostBtn, P.A_EN, v => !!v); return boostBtn; })()
          )
        );

        const speedSection = h('div', { class: 'section' },
        ...
        const toolsSection = h('div', { class: 'section' },
          h('div', { class: 'section-label' }, '도구'),
          h('div', { class: 'row' },
            h('button', { class: 'btn', onclick: async () => { const v = window.__VSC_APP__?.getActiveVideo(); if (v) await togglePiPFor(v); } }, h('span', { class: 'icon' }, '📺'), 'PiP'),
            (() => { const zoomBtn = h('button', { class: 'btn' }, h('span', { class: 'icon' }, '🔍'), '줌'); zoomBtn.onclick = () => { setAndHint(P.APP_ZOOM_EN, !sm.get(P.APP_ZOOM_EN)); }; bindClassToggle(zoomBtn, P.APP_ZOOM_EN, v => !!v); return zoomBtn; })()
          )
        );

        const advToggleBtn = h('button', { class: 'btn btn-sm', style: 'width:100%;margin-bottom:4px;color:rgba(255,255,255,0.5);' });
        ...
        const body = h('div', { class: 'body' }, videoSection, audioSection, speedSection, toolsSection, advToggleBtn, advContainer);

        const footer = h('div', { class: 'footer' },
          h('button', { class: 'btn btn-sm', onclick: () => sm.set(P.APP_UI, false) }, '✕ 닫기'),
          (() => { const pwrBtn = h('button', { class: 'btn btn-sm' }); pwrBtn.onclick = () => setAndHint(P.APP_ACT, !sm.get(P.APP_ACT)); bindStyle(pwrBtn, P.APP_ACT, (el, v) => { el.className = 'btn btn-sm ' + (v ? 'success' : 'danger'); el.textContent = v ? '⚡ ON' : '⚡ OFF'; }); return pwrBtn; })(),
        ...
        const mainPanel = h('div', { class: 'panel' }, dragHandle, body, footer);
    ...
    function getRateState(v) { const st = getVState(v); if (!st.rateState) { st.rateState = { orig: null, lastSetAt: 0 }; } return st.rateState; }
    function markInternalRateChange(v) { const st = getRateState(v); st.lastSetAt = performance.now(); }
    const restoreRateOne = (el) => { try { const st = getRateState(el); if (!st || st.orig == null) return; const nextRate = Number.isFinite(st.orig) && st.orig > 0 ? st.orig : 1.0; markInternalRateChange(el); el.playbackRate = nextRate; st.orig = null; } catch (_) {} };
      ...
      return {
        apply(video, mode, vVals) {
          ...
          if (st.fxBackend && st.fxBackend !== effectiveMode) {
            if (st.fxBackend === 'webgl' && effectiveMode === 'svg') { showOSD('WebGL → SVG 자동 전환', 2000); }
          }
    ...
    const bindVideoOnce = (v, ApplyReq) => {
      ...
      const softResetTransientFlags = () => { st.audioFailUntil = 0; st.rect = null; st.rectT = 0; st.rectEpoch = -1; st.webglFailCount = 0; st.webglDisabledUntil = 0; st.webglTainted = false; if (st.rateState) { st.rateState.orig = null; st.rateState.lastSetAt = 0; } st.desiredRate = undefined; ApplyReq.hard(); };
    ...
    /* ── v171.1.0 patch: mainTarget visibility bypass ── */
    ...
    function createAppController({ Store, Registry, Scheduler, ApplyReq, Adapter, Audio, UI, Utils, P, Targeting }) {
      ...
      function updateQualityScale(v) {
        ...
        const now = performance.now(); if (now - lastQCheck < 2000) return qualityScale; lastQCheck = now;
        try {
          const q = v.getVideoPlaybackQuality(); const dropped = Number(q.droppedVideoFrames || 0); const total = Number(q.totalVideoFrames || 0); const ratio = total > 0 ? (dropped / total) : 0;
          const prev = qualityScale;
          qualityScale = ratio > 0.08 ? 0.65 : ratio > 0.04 ? 0.80 : 1.0;
          if (qualityScale < prev && qualityScale < 0.9) { showOSD(`⚠ 프레임 드롭 감지 → 효과 자동 축소 (${Math.round(qualityScale * 100)}%)`, 2500); }
      ...
      Scheduler.registerApply((force) => {
        try {
          ...
          /* ── v171.1.0 patch: pass mainTarget to reconcile ── */
