// ==UserScript==
// @name         Video_Control (v170.9.3 - Zero-Idle WebGL & Adaptive SVG)
...
(function () {
  ...
  function VSC_MAIN() {
    ...
    const VSC_DEFENSE = Object.freeze({ webglCooldown: true, audioCooldown: true, autoSceneDrmBackoff: true, hideAmbientGlow: false });
    ...
    const DEFAULTS = { video: { presetS: 'off', presetB: 'off', presetMix: 1.0, shadowBandMask: 0, brightStepLevel: 0 }, audio: { enabled: false, boost: 6 }, playback: { rate: 1.0, enabled: false }, app: { active: true, uiVisible: false, applyAll: false, renderMode: 'svg', zoomEn: false, autoScene: false, advanced: false } };
    ...
    function ensureFsWrapper(video) { if (fsWraps.has(video)) return fsWraps.get(video); if (!video || !video.parentNode) return null; const parent = video.parentNode; if (parent.nodeType === 11) return null; const wrap = document.createElement('div'); wrap.className = 'vsc-fs-wrap'; wrap.style.cssText = `position: relative; display: inline-block; width: 100%; height: 100%; max-width: 100%; background: black;`; const ph = document.createComment('vsc-video-placeholder'); parent.insertBefore(ph, video); parent.insertBefore(wrap, video); wrap.appendChild(video); wrap.__vscPlaceholder = ph; fsWraps.set(video, wrap); return wrap; }
    ...
    function onFsChange() { const fsEl = document.fullscreenElement || document.webkitFullscreenElement; if (!fsEl) { for (const v of TOUCHED.videos) { if (fsWraps.has(v)) restoreFromFsWrapper(v); } } if (window.__VSC_UI_Ensure) window.__VSC_UI_Ensure(); }
    ...
    function getActivePiPVideo() { if (document.pictureInPictureElement instanceof HTMLVideoElement) return document.pictureInPictureElement; if (__activeDocumentPiPWindow && __activeDocumentPiPVideo?.isConnected) return __activeDocumentPiPVideo; return null; }
    ...
    async function enterPiP(video) {
      ...
      if (document.pictureInPictureElement === video) return true; if (document.pictureInPictureEnabled && typeof video.requestPictureInPicture === 'function') { try { await video.requestPictureInPicture(); return true; } catch (e) { return false; } } return false;
    }
    async function exitPiP(preferredVideo = null) { if (__activeDocumentPiPWindow) { __activeDocumentPiPWindow.close(); return true; } if (document.pictureInPictureElement && document.exitPictureInPicture) { try { await document.exitPictureInPicture(); return true; } catch (_) {} } return false; }
    ...
    function composeBaseVideoParams(out, vUser) {
      ...
      const pB = PRESETS.grade[vUser.presetB] || PRESETS.grade.off;
...
function normalizeBySchema(sm, schema) { let changed = false; const setIfDiff = (path, val) => { if (!Object.is(sm.get(path), val)) { sm.set(path, val); changed = true; } }; for (const rule of schema) { const type = rule.type; const path = rule.path; if (type === 'bool') { setIfDiff(path, !!sm.get(path)); continue; } if (type === 'enum') { const cur = sm.get(path); if (!rule.values.includes(cur)) { setIfDiff(path, rule.fallback()); } continue; } if (type === 'num') { let n = Number(sm.get(path)); if (!Number.isFinite(n)) n = rule.fallback(); if (rule.round) n = Math.round(n); n = Math.max(rule.min, Math.min(rule.max, n)); setIfDiff(path, n); continue; } } return changed; }
    ...
    function createAudio(sm) {
      ...
      const buildAudioGraph = () => { compressor = ctx.createDynamicsCompressor(); compressor.threshold.value = -18; compressor.knee.value = 12; compressor.ratio.value = 3.0; compressor.attack.value = 0.003; compressor.release.value = 0.15; limiter = ctx.createDynamicsCompressor(); limiter.threshold.value = -1.5; limiter.knee.value = 0.0; limiter.ratio.value = 20.0; limiter.attack.value = 0.0015; limiter.release.value = 0.09; hpf = ctx.createBiquadFilter(); hpf.type = 'highpass'; hpf.frequency.value = VSC_AUD_HPF_HZ; hpf.Q.value = VSC_AUD_HPF_Q; clipper = ctx.createWaveShaper(); clipper.curve = getSoftClipCurve(); try { clipper.oversample = '2x'; } catch (_) {} analyser = ctx.createAnalyser(); analyser.fftSize = 2048; dataArray = new Float32Array(analyser.fftSize); kWeightShelf = ctx.createBiquadFilter(); kWeightShelf.type = 'highshelf'; kWeightShelf.frequency.value = 1500; kWeightShelf.gain.value = 4.0; kWeightPass = ctx.createBiquadFilter(); kWeightPass.type = 'highpass'; kWeightPass.frequency.value = 38; kWeightPass.Q.value = 0.7071; dryOut = ctx.createGain(); wetOut = ctx.createGain(); wetInGain = ctx.createGain(); masterOut = ctx.createGain(); dryOut.connect(masterOut); wetOut.connect(masterOut); hpf.connect(compressor); hpf.connect(kWeightShelf); compressor.connect(wetInGain); wetInGain.connect(limiter); limiter.connect(clipper); clipper.connect(wetOut); kWeightShelf.connect(kWeightPass); kWeightPass.connect(analyser); masterOut.connect(ctx.destination); };
    ...
    function createFiltersVideoOnly(Utils, config) {
      ...
      function buildSvg(root) {
        ...
        const tone = mkXfer({ in: 'mixedSharpSum', result: 'toned' }, { type: 'table', tableValues: '0 1' });
        const lumBase = h('feColorMatrix', { ns: 'svg', in: 'SourceGraphic', type: 'matrix', values: '0.2126 0.7152 0.0722 0 0 0.2126 0.7152 0.0722 0 0 0.2126 0.7152 0.0722 0 0 0 0 0 1 0', result: 'lumBase' });
        ...
        const b1 = h('feGaussianBlur', { ns: 'svg', in: 'SourceGraphic', stdDeviation: '0', result: 'b1' });
        const sh1 = h('feComposite', { ns: 'svg', in: 'SourceGraphic', in2: 'b1', operator: 'arithmetic', k2: '1', k3: '0', result: 'sh1' });
        ...
        const gamMasked = h('feComposite', { ns: 'svg', in: 'SourceGraphic', in2: 'invGate', operator: 'arithmetic', k1: '1', result: 'gamMasked' });
        ...
        const tmp = mkXfer({ in: 'toned', result: 'tmp' }, { type: 'linear', slope: '1', intercept: '0' });
        const sat = h('feColorMatrix', { ns: 'svg', in: 'tmp', type: 'saturate', values: '1', result: 'sat' });
        full.append(lumBase, rngMax, rngMin, rng, rngBlur, edgeGate, hlKeep, gate, b1, sh1, b2, sh2, bc, cl, sharpDesat, sharpBiased, invGate, gamMasked, sharpMasked, mixedSharpSum, tone, tmp, sat);
      ...
      function prepare(video, s) {
        ...
        if (ctx.st.lastKey !== key) {
          ...
          let morphRadius = 0; if (filterGrade === 'F' && sharpN > 0.15) morphRadius = 1;
...
void main() {
  ...
  outColor = vec4(clamp(color, 0.0, 1.0), 1.0);
      ...
      class WebGLPipeline {
        ...
        scheduleNextFrame(loopFn) { if (document.hidden) { this._loopRunning = false; return; } if (this.video?.paused) { if (this._paramsDirty) { this._loopRunning = false; requestAnimationFrame(() => { this.render(); }); } else { this._loopRunning = false; } return; } this.renderDriver.scheduleVideoFrame(this.video, loopFn, 'raf', 90); }
...
const __styleCache = new Map();
    ...
    function showOSD(text, durationMs = 1200) {
      ...
      if (!__osdEl || !__osdEl.isConnected || __osdEl.parentNode !== root) {
        ...
        __osdEl.style.cssText = `position: fixed; top: 60px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.82); color: #fff; padding: 8px 20px; border-radius: 8px; font: 600 14px/1.4 sans-serif; z-index: 2147483647; pointer-events: none; opacity: 0; transition: opacity 0.2s; backdrop-filter: blur(6px);`;
        root.appendChild(__osdEl);
      }
      __osdEl.textContent = text; __osdEl.style.opacity = '1';
      clearTimeout(__osdEl._timer);
      __osdEl._timer = setTimeout(() => { __osdEl.style.opacity = '0'; }, durationMs);
    }
    ...
    const bindVideoOnce = (v, ApplyReq) => {
      ...
      const softResetTransientFlags = () => { st.audioFailUntil = 0; st.rect = null; st.rectT = 0; st.rectEpoch = -1; st.webglFailCount = 0; st.webglDisabledUntil = 0; st.webglTainted = false; if (st.rateState) { st.rateState.orig = null; st.rateState.lastSetAt = 0; st.rateState.suppressSyncUntil = 0; } st.desiredRate = undefined; ApplyReq.hard(); };
