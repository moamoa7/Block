// ==UserScript==
// @name        Video_Control (v159.10.2_Targeting_PointerFix)
// @namespace   https://github.com/
// @version     159.10.2
// @description Video Control: Ultra-Lightweight. Strict renderable targeting & Pointer Events for drag stuck fix.
// @match       *://*/*
...
  const CONFIG = Object.freeze({ VERSION: "v159.10.2", IS_MOBILE: detectMobile(), IS_LOW_END: __IS_LOW_END, TOUCHED_MAX: __IS_LOW_END ? 60 : 140, VSC_ID: (globalThis.crypto?.randomUUID?.() || Math.random().toString(36).slice(2)).replace(/-/g, ""), DEBUG: false });

  const FEATURE_FLAGS = Object.freeze({ lightweightMode: true, trackShadowRoots: true, iframeInjection: true, zoomFeature: false });
  const PERF_POLICY = Object.freeze({ svg: { useLiteFilterByDefault: true, detailThresholdLowEnd: 8, detailThresholdNormal: 3 }, webgl: { colorOnlySharpThresholdLowEnd: 0.06, colorOnlySharpThresholdNormal: 0.03 }, registry: { shadowLRUMaxLowEnd: 4, shadowLRUMaxNormal: 12, spaRescanDebounceMs: 220 } });
...
  function getVState(v) { let st = videoStateMap.get(v); if (!st) { st = { visible: false, rect: null, ir: 0, bound: false, rateState: null, tainted: false, audioFailUntil: 0, applied: false, fxBackend: null, forceSvg: false, desiredRate: undefined, lastFilterUrl: null, rectT: 0, rectEpoch: -1, fsPatched: false, webglFailCount: 0, webglDisabledUntil: 0 }; videoStateMap.set(v, st); } return st; }
...
  function hashApplySet(set) { let sum = 0 >>> 0, sumSq = 0 >>> 0, xor = 0 >>> 0, n = 0; for (const el of set) { const id = (getElemId(el) | 0) >>> 0; n++; sum = (sum + id) >>> 0; sumSq = (sumSq + Math.imul(id, id)) >>> 0; let h = id >>> 0; h ^= h >>> 16; h = Math.imul(h, 0x7feb352d) >>> 0; h ^= h >>> 15; h = Math.imul(h, 0x846ca68b) >>> 0; h ^= h >>> 16; xor ^= h; } return `${n}:${sum.toString(36)}:${sumSq.toString(36)}:${xor.toString(36)}`; }
...
  // ✅ 타겟팅 필터링 기준 강화 (투명/숨김/크기불량 비디오 차단)
...
              const key = `${Store.rev()}|${rMode}|${w}x${h}|hdr:${hdr}|${press.toFixed(2)}`;

              if (key === lastKey && lastSvg && lastWebgl) { return rMode === 'webgl' ? lastWebgl : lastSvg; }
              const base = {};
              composeVideoParamsInto(base, vfUser, Utils, rMode === 'webgl' ? 'webgl' : 'svg', activeTarget);
              const scaled = mtMonitor.applyScale(base);

              lastSvg = scaled;
              lastWebgl = projectVValsForWebGL(scaled, vfUser.tonePreset);
              lastKey = key;
...
    class WebGLPipeline {
      constructor() {
        this.canvas = null; this.gl = null; this.activeProgramKind = ''; this.videoTexture = null; this.video = null; this.active = false; this.vVals = null; this.originalParent = null; this.originalNextSibling = null; this.restoreVideoStyle = null; this.renderDriver = createFrameDriver(); this.disabledUntil = 0;
...
      const isWebGL = sm.get(P.APP_RENDER_MODE) === 'webgl'; const dragHandle = h('div', { class: 'header' }, 'VSC 렌더링 제어');
...
      let dragging = false, dragThresholdMet = false, startY = 0, startX = 0, baseTop = 0, baseLeft = 0, activePointerId = null;
      function endDrag() { dragging = false; dragThresholdMet = false; activePointerId = null; mainPanel.style.transition = ''; }
      dragHandle.addEventListener('pointerdown', (e) => {
        if (e.button != null && e.button !== 0) return;
        dragging = true; dragThresholdMet = false; activePointerId = e.pointerId;
        const rect = mainPanel.getBoundingClientRect(); startY = e.clientY; startX = e.clientX; baseTop = rect.top; baseLeft = rect.left;
        mainPanel.style.transform = 'none'; mainPanel.style.top = `${baseTop}px`; mainPanel.style.right = 'auto'; mainPanel.style.left = `${baseLeft}px`;
        try { dragHandle.setPointerCapture?.(e.pointerId); } catch (_) {}
      });
      dragHandle.addEventListener('pointermove', (e) => {
        if (!dragging) return; if (activePointerId != null && e.pointerId !== activePointerId) return;
        const dy = e.clientY - startY, dx = e.clientX - startX;
        if (!dragThresholdMet && (Math.abs(dx) > 5 || Math.abs(dy) > 5)) { dragThresholdMet = true; mainPanel.style.transition = 'none'; if (e.cancelable) e.preventDefault(); }
        if (!dragThresholdMet) return;
        const panelRect = mainPanel.getBoundingClientRect();
        let nextLeft = Math.max(0, Math.min(window.innerWidth - panelRect.width, baseLeft + dx)), nextTop = Math.max(0, Math.min(window.innerHeight - panelRect.height, baseTop + dy));
        mainPanel.style.left = `${nextLeft}px`; mainPanel.style.top = `${nextTop}px`;
        if (e.cancelable) e.preventDefault();
      });
      dragHandle.addEventListener('pointerup', (e) => { if (activePointerId != null && e.pointerId !== activePointerId) return; endDrag(); });
      dragHandle.addEventListener('pointercancel', endDrag); dragHandle.addEventListener('lostpointercapture', endDrag);
...
      // ✅ Pointer Events 기반 기어 드래그
      let dragging = false, dragThresholdMet = false, startY = 0, baseTop = 0, activePointerId = null;
      function endDrag() { dragging = false; dragThresholdMet = false; activePointerId = null; if (gearBtn) gearBtn.style.transition = ''; }
      function onPointerDown(e) { if (e.button != null && e.button !== 0) return; if (e.target !== gearBtn) return; dragging = true; dragThresholdMet = false; activePointerId = e.pointerId; const rect = gearBtn.getBoundingClientRect(); startY = e.clientY; baseTop = rect.top; try { gearBtn.setPointerCapture?.(e.pointerId); } catch (_) {} }
      function onPointerMove(e) { if (!dragging) return; if (activePointerId != null && e.pointerId !== activePointerId) return; const dy = e.clientY - startY; if (!dragThresholdMet && Math.abs(dy) > 10) { dragThresholdMet = true; gearBtn.style.transition = 'none'; gearBtn.style.transform = 'none'; gearBtn.style.top = `${baseTop}px`; if (e.cancelable) e.preventDefault(); } if (!dragThresholdMet) return; let newTop = baseTop + dy; const rect = gearBtn.getBoundingClientRect(); newTop = Math.max(0, Math.min(window.innerHeight - rect.height, newTop)); gearBtn.style.top = `${newTop}px`; if (e.cancelable) e.preventDefault(); }
      function onPointerUp(e) { if (activePointerId != null && e.pointerId !== activePointerId) return; const wasDragging = dragThresholdMet; endDrag(); if (wasDragging) { e.preventDefault?.(); e.stopPropagation?.(); } }
      function onPointerCancel() { endDrag(); } function onLostPointerCapture() { endDrag(); }

      gearBtn.onclick = (e) => { if (dragThresholdMet) { e.preventDefault(); e.stopPropagation(); return; } setAndHint(P.APP_UI, !sm.get(P.APP_UI)); };
      gearBtn.addEventListener('pointerdown', onPointerDown, { passive: true }); gearBtn.addEventListener('pointermove', onPointerMove, { passive: false }); gearBtn.addEventListener('pointerup', onPointerUp, { passive: false }); gearBtn.addEventListener('pointercancel', onPointerCancel, { passive: true }); gearBtn.addEventListener('lostpointercapture', onLostPointerCapture, { passive: true });
      onWin('blur', endDrag, { passive: true, signal: uiWakeCtrl.signal }); onDoc('visibilitychange', () => { if (document.visibilityState !== 'visible') endDrag(); }, { passive: true, signal: uiWakeCtrl.signal });
...
    const softResetTransientFlags = () => { st.tainted = false; st.audioFailUntil = 0; st.rect = null; st.rectT = 0; st.rectEpoch = -1; if (st.rateState) { st.rateState.orig = null; st.rateState.lastSetAt = 0; st.rateState.suppressSyncUntil = 0; } vscSignal({ forceApply: true }); };
...
      if (videoFxOn) {
          const finalMode = st.forceSvg ? 'svg' : rMode;
          const finalVVals = finalMode === 'webgl' ? vValsPair.webgl : vValsPair.svg;
...
        };
        debugLogEffectiveVVals(rMode === 'webgl' ? vValsPair.webgl : vValsPair.svg, vfEff, __activeTarget, rMode);
