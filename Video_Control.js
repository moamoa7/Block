// ==UserScript==
// @name         Video_Control (v178.9.12 - Pure & Clean)
// @namespace    https://github.com/
// @version      178.9.12
// @description  Video Control: SmartSeek 적용, 가독성/안정성 최적화.
// @match        *://*/*
...
(function () {
  ...
  function VSC_MAIN() {
    ...
    const VSC_BOOT_KEY = Symbol.for('VSC_BOOT_LOCK_178.9.12');
    if (window[VSC_BOOT_KEY]) return;
    window[VSC_BOOT_KEY] = true;

    const VSC_NS_NEW = Symbol.for('__VSC__');
    if (!window[VSC_NS_NEW]) {
      window[VSC_NS_NEW] = {};
    }
    const __vscNs = window[VSC_NS_NEW];
    __vscNs.__version = '178.9.12';
    ...
    function on(target, type, fn, opts = {}) {
      ...
      try {
        target.addEventListener(type, fn, { ...merged, signal: sig });
      } catch (_) {
        try {
    ...
    const getSmoothStroke = (color = '#000') =>
      `text-shadow: 1px 1px 0 ${color}, -1px -1px 0 ${color}, 1px -1px 0 ${color}, -1px 1px 0 ${color}, 0px 1px 0 ${color}, 0px -1px 0 ${color}, 1px 0px 0 ${color}, -1px 0px 0 ${color};`;
    ...
    function installShadowRootEmitterIfNeeded() {
      ...
      if (!__vscNs._origAttachShadowDesc) {
        __vscNs._origAttachShadowDesc = desc;
      }
    ...
    const DEFAULTS = {
      ...
      app: {
        active: true,
        uiVisible: false,
        applyAll: true, // 사용자가 설정한 기본값 (true)로 적용
        zoomEn: false,
        autoScene: false,
        autoScenePreset: 'Normal',
        advanced: false,
        timeEn: true,
        timePos: 1
      }
    ...
    const P = Object.freeze({
      ...
      APP_ZOOM_EN: 'app.zoomEn', APP_AUTO_SCENE: 'app.autoScene', APP_AUTO_SCENE_PRESET: 'app.autoScenePreset',
      APP_ADV: 'app.advanced', APP_TIME_EN: 'app.timeEn', APP_TIME_POS: 'app.timePos',
    ...
    const APP_SCHEMA = [
      ...
      { type: 'enum', path: P.APP_AUTO_SCENE_PRESET, values: ['Soft', 'Normal', 'Strong'], fallback: () => 'Normal' },
    ...
    function createLocalStore(defaults, scheduler, Utils) {
      ...
      function _doSave() {
        ...
        try {
          ...
          if (json.length > 8192) {
            log.warn('Settings too large, skipping save');
            return;
          }
      ...
      const emit = (path, val) => {
        ...
        if (cbs) {
          for (const cb of cbs) safe(() => cb(val));
        }
        ...
        if (dot > 0) {
          ...
          if (cbsStar) {
            for (const cb of cbsStar) safe(() => cb(val));
          }
        }
      };
...
function restoreFromDocumentPiP(video) {
  ...
  try {
    ...
    if (!restored) {
      ...
      if (fallbackContainer) fallbackContainer.appendChild(video);
...
function createTargeting() {
  ...
  function pickFastActiveOnly(videos, lastUserPt, audioBoostOn) {
    ...
    const evalScore = (v) => {
      ...
      if (!v.paused && !v.ended) {
        s += 6.0;
      } else if (v.currentTime > 5.0 && (v.duration || 0) > 30) {
        s += 3.0;
      }
      ...
      const dx = cx - lastUserPt.x;
      const dy = cy - lastUserPt.y;
      ...
      if (!v.controls && !v.closest(PLAYER_CONTAINER_SELECTORS)) s -= 1.0;
...
function createRegistry(scheduler) {
  ...
  const visible = { videos: new Set() };
  let dirtyA = { videos: new Set() };
  let dirtyB = { videos: new Set() };
  let dirty = dirtyA;
  let rev = 0;
    ...
    for (const e of entries) {
      ...
      if (e.contentBoxSize?.[0]) {
        ...
        st.rect = {
          ...
          left: st.rect?.left ?? 0,
          top: st.rect?.top ?? 0,
          right: (st.rect?.left ?? 0) + s.inlineSize,
          bottom: (st.rect?.top ?? 0) + s.blockSize
        };
      } else {
  ...
  const refreshObservers = () => {
    for (const mo of observerMap.values()) {
      mo.disconnect();
    }
    observerMap.clear();
...
function createAudio(sm) {
  ...
  const stt = (param, val, t, tc = 0.08) => {
    if(param) {
      try { param.setTargetAtTime(val, t, tc); }
      catch (_) { param.value = val; }
    }
  };
  ...
  function createDynamicCinemaEQ(actx) {
    const bands = {
      sub: mkBQ(actx, 'lowshelf', 80, 0.8, 0),
      impact: mkBQ(actx, 'peaking', 55, 1.2, 0),
      cut: mkBQ(actx, 'peaking', 300, 0.8, 0),
      voice: mkBQ(actx, 'peaking', 3200, 1.2, 0),
    ...
    const applyGains = () => {
      const profile = PROFILES[activeProfile] || PROFILES.neutral;
  ...
  function buildMultibandDynamics(actx) {
    ...
    const input = actx.createGain();
    const lpLow = createLR4(CROSSOVER_LOW, 'lowpass');
    const hpLow = createLR4(CROSSOVER_LOW, 'highpass');
    const lpMid = createLR4(CROSSOVER_HIGH, 'lowpass');
    const hpHigh = createLR4(CROSSOVER_HIGH, 'highpass');

    input.connect(lpLow.input);
    input.connect(hpLow.input);
    hpLow.output.connect(lpMid.input);
    hpLow.output.connect(hpHigh.input);
    ...
    gainHigh.connect(output);

    return {
      input, output,
      bands: {
        low: { comp: compLow, gain: gainLow },
        mid: { comp: compMid, gain: gainMid },
        high: { comp: compHigh, gain: gainHigh }
      }
    };
  }
  ...
  function createLUFSMeter(actx) {
    ...
    const mMean = new Float32Array(M_N);
    const mDt = new Float32Array(M_N);
    const sMean = new Float32Array(S_N);
    const sDt = new Float32Array(S_N);
    ...
    function measure() {
      ...
      if (state.momentaryLUFS > -70 && state.momentaryLUFS > state.integratedLUFS - 10) {
        state.integratedSum += meanSq;
        state.integratedCount++;
    ...
    return {
      ...
      reset: () => {
        ...
        Object.assign(state, {
          mIdx:0, mFill:0, mSumW:0, mSumDt:0, sIdx:0, sFill:0, sSumW:0, sSumDt:0,
          integratedSum:0, integratedCount:0, momentaryLUFS:-70, shortTermLUFS:-70, integratedLUFS:-70
        });
      },
      getState: (out) => {
        if (!out) return {
          momentaryLUFS: state.momentaryLUFS,
          shortTermLUFS: state.shortTermLUFS,
  ...
  function createLoudnessNormalizer(actx, lufsMeter) {
    ...
    function update() {
      const lufs = lufsMeter.getState(_tmp);
      frameCount++;
  ...
  function createDialogueBoostProfile() {
    ...
    return {
      getProfile(enabled) {
        return enabled ? PROFILES.dialogueBoost : PROFILES.off;
      }
    };
  }
  ...
  function buildAudioGraph(audioCtx) {
    const n = {
      ...
      hpf: mkBQ(audioCtx, 'highpass', 35, 0.707),
      limiter: mkComp(audioCtx, -1.0, 0.0, 20.0, 0.001, 0.08),
      clipper: audioCtx.createWaveShaper()
    };

    n.clipper.curve = getSoftClipCurve();
    try { n.clipper.oversample = '2x'; } catch (_) {}

    const dynamicEQ = createDynamicCinemaEQ(audioCtx);
    const multiband = buildMultibandDynamics(audioCtx);
    const lufsMeter = createLUFSMeter(audioCtx);
    const loudnessNorm = createLoudnessNormalizer(audioCtx, lufsMeter);

    n._dialogueProfile = createDialogueBoostProfile();
    n.wetInGain = loudnessNorm.node;

    n.inputGain.connect(n.dryGain);
    n.dryGain.connect(n.masterOut);
    ...
    n._dynamicEQ = dynamicEQ;
    n._multiband = multiband;
    n._lufsMeter = lufsMeter;
    n._loudnessNorm = loudnessNorm;
  ...
  const ensureCtx = () => {
    ...
    dryGain = nodes.dryGain;
    wetGain = nodes.wetGain;
    masterOut = nodes.masterOut;
    wetInGain = nodes.wetInGain;
  ...
  const fadeOutThen = (fn) => {
    ...
    try {
      ...
      masterOut.gain.linearRampToValueAtTime(0, t + fadeMs / 1000);
    ...
    switchTimer = setTimeout(() => {
      ...
      if (ctx) {
        ...
        try {
          ...
          masterOut.gain.linearRampToValueAtTime(1, t2 + fadeMs / 1000);
  ...
  function runAudioLoop(tok) {
    ...
    if (currentSrc && currentNodes) {
      ...
      if (actuallyEnabled) {
        ...
        const stLufs = lufsSt.shortTermLUFS;
        const intLufs = lufsSt.integratedLUFS;
    ...
    if (document.hidden) {
      ...
      if (target && !target.ended) {
        ...
        currentTarget.addEventListener('play', resume, { once: true });
        currentTarget.addEventListener('seeked', resume, { once: true });
  ...
  const updateMix = () => {
    ...
    const wetTarget = (dynAct && isHooked) ? 1 : 0;
    const dryTarget = 1 - wetTarget;

    stt(dryGain.gain, dryTarget, ctx.currentTime, 0.005);
    stt(wetGain.gain, wetTarget, ctx.currentTime, 0.005);
    ...
    if (currentNodes) {
      ...
      if (currentNodes._multiband) {
        const mb = currentNodes._multiband.bands;
        const t = ctx.currentTime;
        stt(mb.low.comp.ratio, mbEnabled ? 2.5 : 1.0, t, 0.02);
        stt(mb.mid.comp.ratio, mbEnabled ? 2.2 : 1.0, t, 0.02);
        stt(mb.high.comp.ratio, mbEnabled ? 1.8 : 1.0, t, 0.02);
      }
  ...
  async function destroy() {
    ...
    ctx = null; currentNodes = null; limiter = null; wetInGain = null;
    inputGain = null; dryGain = null; wetGain = null; masterOut = null; hpf = null;
...
function createFiltersVideoOnly(Utils, config) {
  ...
  function buildSvg(root) {
    ...
    const mkC = (p) => {
      const b = h('feComponentTransfer', { ns: 'svg', result: `${p}_b` }, mkFuncRGB({ type: 'linear', slope: '1', intercept: '0' }));
    ...
    const cL = mkC('l');
    const pL = mkP('l', 'l_g');
    lite.append(cL.b, cL.g, pL.tmp.tm, pL.s);
    ...
    sharp.append(cS.b, cS.g, sYR, sUV, yBlur, yDiff, ySharp, yuv, toRgb, pS.tmp.tm, pS.s);
    ...
    const commonByTier = {
      lite: { bcLinFuncs: Array.from(cL.b.children), gamFuncs: Array.from(cL.g.children), tmp: pL.tmp, sats: [pL.s] },
      sharp: { bcLinFuncs: Array.from(cS.b.children), gamFuncs: Array.from(cS.g.children), tmp: pS.tmp, sats: [pS.s] }
    };
    ...
    return {
      ...
      sharpDetail,
      st: {
        lastKey: '', bcLinKey: '', gammaKey: '', tempKey: '', satKey: '',
        commonTier: {
          lite: { bcLinKey: '', gammaKey: '', tempKey: '', satKey: '' },
          sharp: { bcLinKey: '', gammaKey: '', tempKey: '', satKey: '' }
        },
        sharpKey: '', rev: 0
      }
    };
  ...
  return {
      ...
      try {
        ...
        if (nodes) {
          ...
          for (const tierKey of ['lite', 'sharp']) {
            ...
            if (cst) {
              cst.bcLinKey = ''; cst.gammaKey = ''; cst.tempKey = ''; cst.satKey = '';
            }
          }
        }
    ...
    prepareCached: (video, s) => {
      try { return prepare(video, s); }
        ...
        log.warn('filter prepare failed:', e);
...
// ✨ smartSeek 함수 추가 (Part 2와 3 사이 전역 유틸리티)
function smartSeek(video, deltaSec) {
  if (!video || video.readyState === 0) return false;

  const isLiveLike = (!Number.isFinite(video.duration) || video.duration === Infinity);
  let minT = 0, maxT = video.duration || 0;

  if (isLiveLike) {
    const sr = video.seekable;
    if (!sr || sr.length === 0) return false;
    minT = sr.start(0);
  ...
  const from = Number(video.currentTime) || minT;
  let target = from + deltaSec;

  const EPS = 0.10;
  if (target > maxT - EPS) target = maxT - EPS;
  ...
  const verifyAndRetry = () => {
    ...
    // 허용 오차(0.5초) 내에 진입했으면 성공
    if (Math.abs(cur - t0) <= 0.5) return;
    if (++tries >= 3) return;
...
// --- PART 3 START ---
  ...
  function createUI(sm, registry, ApplyReq, Utils, P) {
    ...
    const syncVVVars = () => {
      try {
        ...
        if (!root) return;
        if (!vv) {
          root.style.setProperty('--__vsc171-vv-top', '0px');
          root.style.setProperty('--__vsc171-vv-h', `${window.innerHeight}px`);
          return;
        }
        root.style.setProperty('--__vsc171-vv-top', `${Math.round(vv.offsetTop)}px`);
        root.style.setProperty('--__vsc171-vv-h', `${Math.round(vv.height)}px`);
      } catch (_) {}
    };
    ...
    const build = () => {
      ...
      pipBtn.onclick = async (e) => {
        e.stopPropagation();
        if (!sm.get(P.APP_ACT)) return;
        const v = getNS()?.App?.getActiveVideo();
        if(v) await togglePiPFor(v);
      };
      bindReactive(pipBtn, [P.APP_ACT], (el, act) => {
      ...
      const pwrBtn = h('button', { class: 'btn', style: 'flex: 1;' }, '⚡ Power');
      pwrBtn.onclick = (e) => {
        e.stopPropagation();
        setAndHint(P.APP_ACT, !sm.get(P.APP_ACT));
      };
      bindReactive(pwrBtn, [P.APP_ACT], (el, v) => {
        el.style.color = v ? '#2ecc71' : '#e74c3c';
        el.classList.toggle('active', !!v);
      }, sm, sub);
      ...
      advToggleBtn.onclick = (e) => {
        e.stopPropagation();
        setAndHint(P.APP_ADV, !sm.get(P.APP_ADV));
      };
      bindReactive(advToggleBtn, [P.APP_ADV], (el, v) => {
      ...
      const advContainer = h('div', { style: 'display: none; flex-direction: column; gap: 0px;' }, [
        ...
        renderButtonRow({
          label: '복구', key: P.V_BRIGHT_STEP, offValue: 0, toggleActiveToOff: true,
          items: [{ text: '1단', value: 1 }, { text: '2단', value: 2 }, { text: '3단', value: 3 }]
        }),
        renderButtonRow({
          label: '밝기', key: P.V_PRE_B, offValue: 'brOFF', toggleActiveToOff: true,
          items: Object.keys(PRESETS.grade).filter(k => k !== 'brOFF').map(k => ({ text: k, value: k }))
        }),
        h('hr'),
        renderButtonRow({
          label: '시계', key: P.APP_TIME_EN, offValue: false, toggleActiveToOff: true,
          items: [{ text: '표시 (전체화면)', value: true }]
        }),
        renderButtonRow({
          label: '위치', key: P.APP_TIME_POS,
          items: [{ text: '좌', value: 0 }, { text: '중', value: 1 }, { text: '우', value: 2 }]
        }),
        h('hr')
      ]);
      ...
      const bodyMain = h('div', { id: 'p-main' }, [
        ...
        renderButtonRow({
          label: '샤프', key: P.V_PRE_S, offValue: 'off', toggleActiveToOff: true,
          items: Object.keys(PRESETS.detail).filter(k => k !== 'off').map(k => ({ text: k, value: k }))
        }),
    ...
    const ensureGear = () => {
      ...
      const wake = () => {
        ...
        if (inFs || getNS()?.CONFIG?.IS_MOBILE) return;
    ...
    const mount = () => {
      ...
      try {
        if (gearHost && gearHost.parentNode !== gearTarget) gearTarget.appendChild(gearHost);
      } catch (_) {
        try { (document.body || document.documentElement).appendChild(gearHost); } catch (__) {}
      }
      try {
        if (container && container.parentNode !== gearTarget) gearTarget.appendChild(container);
      } catch (_) {
    ...
    return {
      ...
      destroy: () => {
        uiUnsubs.forEach(u => safe(u));
        uiUnsubs.length = 0;
        safe(() => uiWakeCtrl.abort());
        clearTimeout(fadeTimer);
  ...
  function createZoomManager(Store, P) {
    ...
    const resetZoom = (v) => {
      if (v) { const st = getSt(v); st.scale = 1; st._cachedPosition = null; st._lastTransition = null; update(v); }
    };
    ...
    on(window, 'dblclick', e => {
      ...
      e.preventDefault(); e.stopPropagation();
      const st = getSt(v);
      if (st.scale === 1) zoomTo(v, 2.5, e.clientX, e.clientY);
      else resetZoom(v);
    }, { capture: true, signal: zsig });
    ...
    return {
      ...
      setEnabled: (en) => { if (en) attachTouchListeners(); },
      destroy: () => {
        ...
        for (const v of zoomedVideos) {
          ...
          if (st && st.zoomed) {
            v.style.transform = ''; v.style.transformOrigin = ''; v.style.cursor = '';
            v.style.zIndex = st.origZIndex; v.style.position = st.origPosition;
            v.style.transition = ''; st.zoomed = false;
          }
        }
        zoomedVideos.clear();
        isPanning = false; pinchState.active = false; activeVideo = null;
  ...
  const bindVideoOnce = (v, ApplyReq) => {
    ...
    const softResetTransientFlags = () => {
      ...
      if (st.rateState) {
        st.rateState.orig = null; st.rateState.lastSetAt = 0; st.rateState.suppressSyncUntil = 0;
        st.rateState._setAttempts = 0; st.rateState._firstAttemptT = 0;
  ...
  function createVideoParamsMemo() {
    const getDetailLevel = (presetKey) => {
      ...
      if (k === 'XL') return 'xl'; if (k === 'L') return 'l'; if (k === 'M') return 'm'; if (k === 'S') return 's';
      return 'off';
    };
    ...
    return {
      get(vfUser) {
        ...
        out._rs = rs; out._gs = gs; out._bs = bs; out.__detailLevel = getDetailLevel(vfUser.presetS);
        return out;
      }
    };
  }
  ...
  function createAppController({ Store, Registry, Scheduler, ApplyReq, Adapter, Audio, UI, Utils, P, Targeting }) {
    ...
    Scheduler.registerApply((force) => {
      try {
        ...
        let vValsEffective = videoParamsMemo.get(vf0);
  ...
  onPageReady(() => {
    ...
    let ZoomManager = createZoomManager(Store, P);
    __vscNs.ZoomManager = ZoomManager;

    const UI = createUI(Store, Registry, ApplyReq, Utils, P);

    const TimerManager = createTimerManager(Store, P);
    __vscNs.TimerManager = TimerManager;
    ...
    on(window, 'keydown', async (e) => {
      ...
      if (e.altKey && e.shiftKey && e.code === 'KeyV') {
        e.preventDefault(); e.stopPropagation();
        safe(() => {
          const st = getNS()?.Store;
          if (st) {
            st.set(P.APP_UI, !st.get(P.APP_UI));
            ApplyReq.hard();
          }
        });
        return;
    ...
    on(document, 'visibilitychange', () => {
      ...
      safe(() => {
        if (document.visibilityState === 'visible') getNS()?.ApplyReq?.hard();
      });
    }, OPT_P);
